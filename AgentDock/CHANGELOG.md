...
### 更名 AgentDock 与多 Agent 扩展预留

- 产品更名为 **AgentDock**（原名 Claude Code Manager）：窗口标题、横幅、
  User-Agent、窗口类、桌面快捷方式、启动日志文件与可执行文件
  （`dist\AgentDock.exe`）全部同步更新；源文件更名为
  `source\AgentDock.c`，资源更名为 `assets\AgentDock.*`。
- 兼容迁移：首次启动回读旧注册表键
  `Software\Fongap\ClaudeCodeManager` 的全部设置（下次保存写入新键
  `Software\Fongap\AgentDock`）；凭据管理器读取时自动回退旧
  `ClaudeCodeManager/` 目标前缀，密钥无需重新录入；旧版桌面快捷方式
  （`ClaudeCodeManager.lnk/.url`）在确认属于本工具后自动清理。
- `CLAUDE.md` 多智能体托管标记保持
  `<!-- ClaudeCodeManager:agent-teams:... -->` 不变，确保已启用机器上的
  标记检测与停用恢复不受更名影响。
- 新增 `CCM_AGENTS` Agent 描述表（显示名、配置目录/文件、协议、探测标签），
  向导应用选择器、档案存储值名、配置路径解析与模型发现探活均由表驱动；
  新增一个 Agent CLI（如 Gemini CLI）只需补一条表项、一个配置写入器、
  一个接管读取器和一种启动模式（见 docs/ARCHITECTURE.md「Adding a new
  agent」）。

### 配置档案与一键切换

- 模型配置向导新增“配置档案”区：每个目标应用（Claude Code / Codex）独立
  维护最多 16 个命名档案，下拉选择即回填全部字段（含密钥），点击“保存配置”
  完成切换，复用现有探测 / 备份 / 原子写入 / 凭据激活链路。
- 档案字段存于注册表 `ProfilesClaude` / `ProfilesCodex`；API 密钥本体仅存
  Windows 凭据管理器稳定槽位（`P<id>`），注册表不落任何明文密钥。
- “存为档案”：未选中时输入名称新建，选中已有档案时确认后覆盖更新；
  删除档案同步清空密钥槽位。
- 现有配置接管：首次打开向导且无档案时自动读取当前生效配置
  （`settings.json` / `config.toml` 与活动凭据）预填表单，一键存为首个档案。
- 保存后日志明确提示生效方式：Claude 热重载即时生效；Codex 需重启终端。
- 新增极简模态输入框（档案命名）；`tests/codex_config_test.c` 扩展档案
  pack/unpack 往返与截断容错用例。

### Codex 双应用支持

- 模型配置向导新增“目标应用”切换（Claude Code / Codex），选择会持久化并在
  下次打开时恢复。
- Codex 配置写入用户级 `~/.codex/config.toml`：行级定点合并只认领顶层
  `model`、`model_reasoning_effort`、`model_provider` 与专属
  `[model_providers.ccm]` 段，其余段落、注释与格式逐字节保留；非法 TOML、
  重复托管键或重复托管段整体拒绝写入。
- Codex 网关密钥存入 Windows 凭据管理器（`CODEX_API_KEY`），`config.toml`
  仅经 `env_key` 引用；不写 `auth.json`，ChatGPT OAuth 登录态不受影响。
  切回官方/仅模型模式时只移除 Manager 托管的 provider 痕迹，用户自有
  provider 一律保留。
- 写入前创建 `config.toml.backup-<时间戳>` 备份，唯一临时文件原子替换。
- 模型发现/探活支持双协议：Claude 走 Anthropic `/v1/messages`（三认证头回退
  保持不变），Codex 走 OpenAI `/v1/responses`（Bearer）；404/405 仍会阻止保存。
- 主窗口新增“启动 Codex”按钮与 `--codex-launcher` 启动器：注入凭据与代理
  环境变量后拉起原生 `codex.exe` / `codex.cmd`。
- “打开配置”现在同时支持 Claude `settings.json` 与 Codex `config.toml`。
- 向导 Codex 视图隐藏 Claude 专属的五角色模型表与上下文预算元数据，改为
  单模型 + 思考强度；接口类型文案与校验按 Responses 语义调整。
- 新增 `tests/codex_config_test.c`（定点合并/幂等/保留性/fail-closed）。
