...
#define CCM_VERSION_MINOR  3
#define CCM_VERSION_PATCH  0
#define CCM_VERSION_TAG    ""             /* suffix tag; bump for re-releases */
#define CCM_VERSION_STRING "1.3.0"       /* semver-ish, User-Agent friendly */
...
#define CCM_VERSION_BANNER L"AgentDock " CCM_VERSION_STRING L"-" CCM_VERSION_TAG
#else
#define CCM_VERSION_BANNER L"AgentDock " CCM_VERSION_STRING
#endif
#define CCM_APP_TITLE          L"AgentDock"
#define CCM_USER_AGENT_BASENAME L"AgentDock/" CCM_VERSION_STRING
...
#define IDC_WIZ_APP 2022
#define IDC_WIZ_PROFILE 2023
#define IDC_WIZ_PROFILE_SAVE 2024
#define IDC_WIZ_PROFILE_DEL 2025
...
static HWND g_launch_codex;
static HWND g_fongap_link;
static HWND g_wizard;
static HWND g_wiz_scope;
static HWND g_wiz_app;
static int g_wiz_app_index;            /* 0 = Claude Code, 1 = Codex */
static DWORD g_cfg_wiz_app;            /* persisted wizard target app */
static int g_model_fetch_app;          /* protocol used by the discovery thread */
static HWND g_wiz_profile;             /* provider profile selector */
static HWND g_wiz_profile_save;
static HWND g_wiz_profile_del;
static int g_wiz_profile_sel;          /* index into g_profiles, -1 = none */
...
static WCHAR g_codex_model[1024];
static WCHAR g_codex_base[2048];
static WCHAR g_codex_secret[4096];
static WCHAR g_codex_effort[64];
static BOOL g_codex_set_provider;
...
static const WCHAR CLASS_NAME[] = L"AgentDockV1";
static const WCHAR WIZARD_CLASS_NAME[] = L"AgentDockModelWizardV1";
static const WCHAR REG_KEY[] = L"Software\\Fongap\\AgentDock";
/* Pre-rename storage location. Read once as a fallback so upgrading users
   keep their workspace, proxy, profile and ownership settings; new writes
   always go to REG_KEY. */
static const WCHAR LEGACY_REG_KEY[] = L"Software\\Fongap\\ClaudeCodeManager";
static const WCHAR AGENT_TEAMS_ENV_NAME[] = L"CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS";
/* Codex (OpenAI CLI) target support. The Manager only ever claims the
 * dedicated "ccm" provider id inside ~/.codex/config.toml; every other
 * section, key and profile in that file belongs to the user and must be
 * preserved verbatim. API keys are referenced via env_key and injected by
 * the launcher, so auth.json is never written. */
#define CCM_CODEX_PROVIDER_ID L"ccm"
#define CCM_CODEX_ENV_NAME    L"CODEX_API_KEY"

/* ---- Agent descriptor table ---------------------------------------------
 * Adding a new agent CLI (e.g. Gemini CLI) starts here: append one entry,
 * give it a config writer next to codex_toml_transform, a takeover reader
 * next to wizard_read_live_codex, its own provider-item strings in
 * wizard_update_app, and a launch mode in run_unified_console_launcher.
 * The wizard app selector, profile store, config paths and the discovery
 * probe all key off this table. */
typedef struct {
    LPCWSTR display;      /* wizard app-combo label */
    LPCWSTR short_name;   /* ASCII suffix for registry value names */
    LPCWSTR config_dir;   /* under %USERPROFILE% */
    LPCWSTR config_file;
    int protocol;         /* 0 = Anthropic Messages, 1 = OpenAI Responses */
    LPCWSTR probe_label;  /* status-line protocol name */
} CCM_AGENT_DESC;
static const CCM_AGENT_DESC CCM_AGENTS[] = {
    {L"Claude Code",      L"Claude", L".claude", L"settings.json", 0, L"Messages"},
    {L"Codex（OpenAI）",  L"Codex",  L".codex",  L"config.toml",   1, L"Responses"},
};
#define CCM_AGENT_COUNT ((int)(sizeof(CCM_AGENTS)/sizeof(CCM_AGENTS[0])))
...
/* ---- Provider profiles (P2) ----
 * A profile is a named, switchable snapshot of one target app's gateway or
 * official setup. Field values live in the registry; the API key itself is
 * stored in Windows Credential Manager under the stable slot "P<id>", so
 * profiles only carry a reference. Switching = load the profile into the
 * wizard and press 保存配置, which reuses the existing probe / backup /
 * atomic-write / credential-activation pipeline unchanged. */
#define CCM_PROFILE_MAX 16
#define CCM_PROFILE_FIELDS 10   /* id, name, kind, base, defm, opus, sonnet, haiku, subagent, effort */
typedef struct {
    DWORD id;
    WCHAR name[64];
    int kind;               /* 0 official, 1 gateway, 2 models-only */
    WCHAR base[2048];
    WCHAR defm[1024];
    WCHAR opus[1024], sonnet[1024], haiku[1024], subagent[1024];
    WCHAR effort[16];
} CCM_PROFILE;
static CCM_PROFILE g_profiles[CCM_PROFILE_MAX];
static int g_profile_count;
static int g_profile_app;   /* app index the loaded list belongs to */
static WCHAR g_profile_pack[262144];
...
static BOOL wends_ci(LPCWSTR s,LPCWSTR suffix);
...
static void startup_log_open(void) {
    ...
    if (!n || n >= 900) return;
    wcat(path, 1024, L"AgentDock-startup.log");
...
static void apply_main_control_fonts(void) {
    HWND controls[] = {g_workspace,g_browse,g_projects,g_refresh,g_launch,g_launch_codex,g_folder,g_network_mode,g_test_network,g_proxy,g_url,g_install,g_import_settings,g_open_settings,g_model_wizard,g_agent_teams,g_shortcut,g_log};
    unsigned int i;
...
static void apply_wizard_control_fonts(void) {
    unsigned int i;
    { HWND wiz[] = {g_wiz_app,g_wiz_profile,g_wiz_profile_save,g_wiz_profile_del,g_wiz_scope,g_wiz_provider,g_wiz_base_url,g_wiz_secret,g_wiz_default_model,g_wiz_opus_model,g_wiz_sonnet_model,g_wiz_haiku_model,g_wiz_subagent_model,g_wiz_effort,g_wiz_test_models,g_wiz_recommended,g_wiz_save,g_wiz_cancel};
...
static void wizard_profile_combo_rebuild(void);
static void wizard_load_live_if_empty(void);
static BOOL agent_config_path(int app, LPWSTR out, unsigned int cap);
...
static void set_busy(BOOL busy) {
    ...
    pEnableWindow(g_launch_codex, !busy);
...
static void load_config(void) {
    ...
    if (pRegOpenKeyExW(HKEY_CURRENT_USER, REG_KEY, 0, KEY_READ, &key) != ERROR_SUCCESS) {
        /* First run after the AgentDock rename: fall back to the legacy
           ClaudeCodeManager key so settings survive; the next save_config()
           persists everything under the new key. */
        if (pRegOpenKeyExW(HKEY_CURRENT_USER, LEGACY_REG_KEY, 0, KEY_READ, &key) != ERROR_SUCCESS) return;
    }
    {
        ...
        reg_read_dword(key,L"WizardApp",&g_cfg_wiz_app);
        if(g_cfg_wiz_app>1)g_cfg_wiz_app=0;
        pRegCloseKey(key);
    }
...
static void save_config(void) {
    ...
    if (pRegCreateKeyExW(HKEY_CURRENT_USER, REG_KEY, 0, 0, 0, KEY_WRITE, 0, &key, &disp) == ERROR_SUCCESS) {
        ...
        pRegSetValueExW(key,L"WizardApp",0,REG_DWORD,(const BYTE*)&g_cfg_wiz_app,sizeof(DWORD));
        pRegCloseKey(key);
    }
}
...
static BOOL get_selected_project(LPWSTR out, unsigned int cap) {
    ...
    WCHAR name[1024];
    if (idx < 0) {
        pMessageBoxW(g_main, L"\u8bf7\u5148\u9009\u62e9\u4e00\u4e2a\u9879\u76ee\u3002", CCM_APP_TITLE, 0x30);
        return FALSE;
    }
    name[0] = 0;
...
/* Codex installs differ (npm shim codex.cmd, native codex.exe, or the
   .local/bin layout shared with claude). Try each in turn; a .cmd is launched
   through cmd.exe by the console launcher. */
static BOOL locate_codex(LPWSTR out, unsigned int cap) {
    WCHAR profile[4096];
    DWORD attrs;
    if (SearchPathW(0, L"codex.exe", 0, cap, out, 0)) {
        attrs=GetFileAttributesW(out);
        if(attrs!=INVALID_FILE_ATTRIBUTES && !(attrs&FILE_ATTRIBUTE_DIRECTORY))return TRUE;
    }
    if (SearchPathW(0, L"codex.cmd", 0, cap, out, 0)) {
        attrs=GetFileAttributesW(out);
        if(attrs!=INVALID_FILE_ATTRIBUTES && !(attrs&FILE_ATTRIBUTE_DIRECTORY))return TRUE;
    }
    if (GetEnvironmentVariableW(L"USERPROFILE", profile, 4096) && profile[0]) {
        wcopy(out, cap, profile);
        wcat(out, cap, L"\\.local\\bin\\codex.exe");
        attrs=GetFileAttributesW(out);
        if (attrs != INVALID_FILE_ATTRIBUTES && !(attrs & FILE_ATTRIBUTE_DIRECTORY)) return TRUE;
    }
    out[0] = 0;
...
static BOOL write_url_shortcut(LPCWSTR desktop, LPCWSTR exe) {
    ...
    wcopy(path,4096,desktop); if(path[wlen(path)-1]!=L'\\') wcat(path,4096,L"\\"); wcat(path,4096,L"AgentDock.url");
...
static BOOL create_desktop_shortcut(BOOL notify) {
    ...
    wcopy(shortcut,4096,desktop); if(shortcut[wlen(shortcut)-1]!=L'\\') wcat(shortcut,4096,L"\\"); wcat(shortcut,4096,L"AgentDock.lnk");
    ...
    /* Pre-rename shortcuts left by ClaudeCodeManager builds. */
    {
        WCHAR legacy[4096];
        wcopy(legacy,4096,desktop); if(legacy[wlen(legacy)-1]!=L'\\') wcat(legacy,4096,L"\\"); wcat(legacy,4096,L"ClaudeCodeManager.lnk");
        if(GetFileAttributesW(legacy)!=INVALID_FILE_ATTRIBUTES && shortcut_points_to_our_exe(legacy)) DeleteFileW(legacy);
        wcopy(legacy,4096,desktop); if(legacy[wlen(legacy)-1]!=L'\\') wcat(legacy,4096,L"\\"); wcat(legacy,4096,L"ClaudeCodeManager.url");
        if(GetFileAttributesW(legacy)!=INVALID_FILE_ATTRIBUTES) DeleteFileW(legacy);
    }
...
fallback_url:
    ...
    if(ok) {
        ...
        append_log(fallback?L"[OK] 已创建桌面启动快捷方式：AgentDock.url":L"[OK] 已创建桌面快捷方式：AgentDock.lnk");
        if(notify) pMessageBoxW(g_main,fallback?L"已在桌面创建 AgentDock 启动快捷方式。\r\n\r\n当前系统未能创建 .lnk，已自动使用兼容的 .url 快捷方式。":L"已在当前用户桌面创建 AgentDock 快捷方式。",CCM_APP_TITLE,MB_ICONINFORMATION);
    } else {
        ...
        if(notify) pMessageBoxW(g_main,L"无法创建桌面快捷方式。\r\n\r\n请确认当前账户可以写入桌面目录。",CCM_APP_TITLE,MB_ICONERROR);
    }
...
static int load_credentials_to_environment(void) {
    ...
    while(names[i]) {
        ...
        if(!name[0])continue;
        target[0]=0; wcat(target,512,L"AgentDock/"); wcat(target,512,name);
        cred=0;
        if(!(CredReadW(target,CRED_TYPE_GENERIC,0,&cred) && cred && cred->CredentialBlob && cred->CredentialBlobSize>=2)) {
            /* Legacy targets saved before the AgentDock rename. */
            if(cred){CredFree(cred);cred=0;}
            target[0]=0; wcat(target,512,L"ClaudeCodeManager/"); wcat(target,512,name);
            CredReadW(target,CRED_TYPE_GENERIC,0,&cred);
        }
        if(cred && cred->CredentialBlob && cred->CredentialBlobSize>=2) {
...
/* Launcher modes shared by launch_in_console and mainCRTStartup. */
#define CCM_LAUNCH_CLAUDE   0
#define CCM_LAUNCH_TERMINAL 1
#define CCM_LAUNCH_CODEX    2

static DWORD run_unified_console_launcher(int mode, LPCWSTR project) {
    ...
    SetCurrentProcessExplicitAppUserModelID(mode==CCM_LAUNCH_CLAUDE?L"Fongap.AgentDock.ClaudeRuntime":(mode==CCM_LAUNCH_CODEX?L"Fongap.AgentDock.CodexRuntime":L"Fongap.AgentDock.Terminal"));
    ...
    SetConsoleTitleW(mode==CCM_LAUNCH_CLAUDE?L"Claude Code":(mode==CCM_LAUNCH_CODEX?L"Codex CLI":L"Claude Code Terminal"));
    ...
    } else if(mode==CCM_LAUNCH_CODEX) {
        if(!locate_codex(app,4096)){MessageBoxW(0,L"未找到 codex（codex.exe / codex.cmd）。请先安装 OpenAI Codex CLI，或在 PATH 中提供。",L"Codex CLI",MB_ICONERROR);if(allocated_console)FreeConsole();return 5;}
        if(wends_ci(app,L".cmd")||wends_ci(app,L".bat")) {
            /* npm-style shim: CreateProcessW cannot exec a batch file directly;
               route through cmd.exe with the shim as the call target. */
            WCHAR comspec[4096], target[4096];
            wcopy(target,4096,app);
            GetEnvironmentVariableW(L"ComSpec",comspec,4096);if(!comspec[0])wcopy(comspec,4096,L"C:\\Windows\\System32\\cmd.exe");
            wcopy(app,4096,comspec);
            cmd[0]=0;
            wcat(cmd,32768,L"\"");wcat(cmd,32768,comspec);wcat(cmd,32768,L"\" /d /c call \"");wcat(cmd,32768,target);wcat(cmd,32768,L"\"");
        } else {
            wcat(cmd,32768,L"\"");wcat(cmd,32768,app);wcat(cmd,32768,L"\"");
        }
        resolve_effective_proxy(g_effective_proxy,1024,desc,256);set_proxy_environment(g_effective_proxy);load_credentials_to_environment();
    ...
    /* CREATE_NEW_PROCESS_GROUP so Ctrl-C inside the console is routed to the
       child (claude / codex / cmd) rather than the launcher, which would leave
       the launcher stuck in WaitForSingleObject. */
    ...
    } else {code=GetLastError();MessageBoxW(0,mode==CCM_LAUNCH_CLAUDE?L"无法启动 Claude Code。":(mode==CCM_LAUNCH_CODEX?L"无法启动 Codex CLI。":L"无法打开终端。"),L"" CCM_APP_TITLE,MB_ICONERROR);}
    if(mode!=CCM_LAUNCH_TERMINAL){clear_loaded_credentials();set_proxy_environment(0);secure_zero_w(g_effective_proxy,1024);}
...
static void launch_in_console(int mode) {
    WCHAR project[4096], exe[4096]; STARTUPINFOW si; PROCESS_INFORMATION pi;
    if(!get_selected_project(project,4096))return;
    if(mode==CCM_LAUNCH_CLAUDE)append_context_budget_summary();
    GetModuleFileNameW(0,exe,4096);memset(&si,0,sizeof(si));memset(&pi,0,sizeof(pi));si.cb=sizeof(si);
    g_cmdline[0]=0;wcat(g_cmdline,32768,L"\"");wcat(g_cmdline,32768,exe);
    wcat(g_cmdline,32768,mode==CCM_LAUNCH_CLAUDE?L"\" --claude-launcher \"":(mode==CCM_LAUNCH_CODEX?L"\" --codex-launcher \"":L"\" --terminal-launcher \""));
    wcat(g_cmdline,32768,project);wcat(g_cmdline,32768,L"\"");
    if(!CreateProcessW(exe,g_cmdline,0,0,FALSE,CREATE_UNICODE_ENVIRONMENT,0,project,&si,&pi)) {
        WCHAR err[32];uint_to_wstr((unsigned int)GetLastError(),err,32);append_log_raw(mode==CCM_LAUNCH_CLAUDE?L"[ERROR] 无法启动 Claude Code，系统错误：":(mode==CCM_LAUNCH_CODEX?L"[ERROR] 无法启动 Codex CLI，系统错误：":L"[ERROR] 无法打开终端，系统错误："));append_log(err);
        pMessageBoxW(g_main,mode==CCM_LAUNCH_CLAUDE?L"无法启动 Claude Code。":(mode==CCM_LAUNCH_CODEX?L"无法启动 Codex CLI。":L"无法打开终端。"),L"" CCM_APP_TITLE,MB_ICONERROR);return;
    }
    CloseHandle(pi.hThread);CloseHandle(pi.hProcess);
    set_status(mode==CCM_LAUNCH_CLAUDE?L"Claude Code 已启动":(mode==CCM_LAUNCH_CODEX?L"Codex CLI 已启动":L"终端已打开"));
}
...
static BOOL save_credential_native(LPCWSTR name,LPCWSTR value) {
    CREDENTIALW_CCM c;WCHAR target[512];if(!name||!name[0]||!value||!value[0])return TRUE;memset(&c,0,sizeof(c));target[0]=0;wcat(target,512,L"AgentDock/");wcat(target,512,name);
    c.Type=CRED_TYPE_GENERIC;c.TargetName=target;c.Comment=L"Stored by AgentDock";c.CredentialBlobSize=wlen(value)*2;c.CredentialBlob=(BYTE*)value;c.Persist=CRED_PERSIST_LOCAL_MACHINE;c.UserName=CCM_APP_TITLE;
...
/* ------------------------------------------------------------------------ */
/* Codex ~/.codex/config.toml editing.                                        */
/*                                                                            */
/* config.toml holds the user's whole Codex setup (sandbox, MCP, hooks,       */
/* profiles), so the Manager never rewrites it wholesale. Instead a           */
/* line-level transform:                                                       */
/*   - top-level `model` is replaced or inserted;                              */
/*   - gateway mode writes top-level `model_provider = "ccm"` and regenerates  */
/*     the Manager-owned `[model_providers.ccm]` section;                      */
/*   - non-gateway modes remove that section (always ours by id) and drop      */
/*     `model_provider` only when its current value is the managed one;        */
/*   - everything else — other sections, profiles, comments, formatting —      */
/*     is copied through byte-for-byte.                                        */
/* Fail-closed: any unterminated string, malformed header or duplicate target  */
/* key aborts the whole edit and no file is touched.                           */
/* ------------------------------------------------------------------------ */

static BOOL toml_validate(LPCWSTR text) {
    int state = 0; /* 0 normal, 1 basic string, 2 literal string, 3 multiline basic, 4 multiline literal */
    unsigned int i = 0;
    if (!text) return FALSE;
    while (text[i]) {
        WCHAR c = text[i];
        if (state == 0) {
            if (c == L'#') { while (text[i] && text[i] != L'\n') i++; continue; }
            if (c == L'\"') { if (text[i+1] == L'\"' && text[i+2] == L'\"') { state = 3; i += 2; } else state = 1; }
            else if (c == L'\'') { if (text[i+1] == L'\'' && text[i+2] == L'\'') { state = 4; i += 2; } else state = 2; }
            else if (c == L'[') {
                unsigned int j = i + 1;
                while (text[j] && text[j] != L']' && text[j] != L'\n') j++;
                if (text[j] != L']') return FALSE; /* unterminated table header */
                i = j;
            }
            else if (c < 0x20 && c != L'\t' && c != L'\n' && c != L'\r') return FALSE;
        }
        else if (state == 1) {
            if (c == L'\\') { i++; if (!text[i] || text[i] == L'\n') return FALSE; }
            else if (c == L'\"') state = 0;
            else if (c == L'\n' || (c < 0x20 && c != L'\t' && c != L'\r')) return FALSE;
        }
        else if (state == 2) {
            if (c == L'\'') state = 0;
            else if (c == L'\n' || (c < 0x20 && c != L'\t' && c != L'\r')) return FALSE;
        }
        else if (state == 3) { if (c == L'\"' && text[i+1] == L'\"' && text[i+2] == L'\"') { state = 0; i += 2; } }
        else { if (c == L'\'' && text[i+1] == L'\'' && text[i+2] == L'\'') { state = 0; i += 2; } }
        i++;
    }
    return state == 0;
}

/* Extract the header name of a "[x.y]" line (trimmed) into out; returns FALSE
   when the line is not a header. Array-of-tables headers "[[x]]" parse to
   "[x]" and can therefore never collide with the managed id. */
static BOOL toml_header_name(LPCWSTR text, unsigned int ls, unsigned int le, LPWSTR out, unsigned int cap) {
    unsigned int i = ls, e = le, o = 0;
    while (i < e && (text[i] == L' ' || text[i] == L'\t')) i++;
    if (i >= e || text[i] != L'[') return FALSE;
    i++;
    while (e > i && (text[e-1] == L' ' || text[e-1] == L'\t' || text[e-1] == L'\r')) e--;
    if (e <= i || text[e-1] != L']') return FALSE;
    e--;
    while (i < e && (text[i] == L' ' || text[i] == L'\t')) i++;
    while (e > i && (text[e-1] == L' ' || text[e-1] == L'\t')) e--;
    while (i < e && o + 1 < cap) out[o++] = text[i++];
    out[o] = 0;
    return o > 0;
}

/* If the line assigns a bare key, copy the key into `key` and the value text
   (trimmed) into `value`; returns FALSE otherwise. Dotted and quoted keys
   fail the bare-key scan and are treated as unknown — safe, because the
   managed keys are always written bare by both Codex and this tool. */
static BOOL toml_line_key_value(LPCWSTR text, unsigned int ls, unsigned int le, LPWSTR key, unsigned int kcap, LPWSTR value, unsigned int vcap) {
    unsigned int i = ls, o = 0, ve;
    while (i < le && (text[i] == L' ' || text[i] == L'\t')) i++;
    while (i < le && o + 1 < kcap) {
        WCHAR c = text[i];
        if (!((c >= L'a' && c <= L'z') || (c >= L'A' && c <= L'Z') || (c >= L'0' && c <= L'9') || c == L'_' || c == L'-')) break;
        key[o++] = c; i++;
    }
    key[o] = 0;
    if (!o) return FALSE;
    while (i < le && (text[i] == L' ' || text[i] == L'\t')) i++;
    if (i >= le || text[i] != L'=') return FALSE;
    i++;
    while (i < le && (text[i] == L' ' || text[i] == L'\t')) i++;
    ve = le;
    while (ve > i && (text[ve-1] == L' ' || text[ve-1] == L'\t' || text[ve-1] == L'\r')) ve--;
    if (ve <= i) return FALSE;
    { unsigned int nchars = ve - i; if (nchars >= vcap) nchars = vcap - 1; memcpy(value, text + i, nchars * 2); value[nchars] = 0; }
    return TRUE;
}

/* Parse a TOML scalar string (basic "..." or literal '...') used for value
   comparison; returns FALSE for anything else (arrays, booleans...). */
static BOOL toml_parse_string_value(LPCWSTR value, LPWSTR out, unsigned int cap) {
    unsigned int n = wlen(value), i = 1, o = 0;
    if (!n) return FALSE;
    if (value[0] == L'\"') {
        if (n < 2 || value[n-1] != L'\"') return FALSE;
        while (i + 1 < n && o + 1 < cap) {
            WCHAR c = value[i++];
            if (c == L'\\' && i + 1 < n) {
                WCHAR e = value[i++];
                if (e == L'n') c = L'\n'; else if (e == L't') c = L'\t'; else if (e == L'r') c = L'\r'; else c = e;
            }
            out[o++] = c;
        }
        out[o] = 0; return TRUE;
    ...
    if (value[0] == L'\'') {
        if (n < 2 || value[n-1] != L'\'') return FALSE;
        while (i + 1 < n && o + 1 < cap) out[o++] = value[i++];
        out[o] = 0; return TRUE;
    }
    return FALSE;
}

static void toml_emit_string(CCM_JSON_OUT *o, LPCWSTR value) {
    unsigned int i = 0;
    jout_char(o, L'\"');
    while (value && value[i]) {
        WCHAR c = value[i++];
        if (c == L'\"' || c == L'\\') { jout_char(o, L'\\'); jout_char(o, c); }
        else if (c == L'\n') jout_text(o, L"\\n");
        else if (c == L'\t') jout_text(o, L"\\t");
        else if (c == L'\r') jout_text(o, L"\\r");
        else jout_char(o, c);
    }
    jout_char(o, L'\"');
}

static void toml_emit_managed_section(CCM_JSON_OUT *o, LPCWSTR base_url) {
    jout_text(o, L"[model_providers." CCM_CODEX_PROVIDER_ID L"]\r\n");
    jout_text(o, L"name = \"AgentDock Gateway\"\r\n");
    jout_text(o, L"base_url = ");
    toml_emit_string(o, base_url && base_url[0] ? base_url : L"https://api.openai.com/v1");
    jout_text(o, L"\r\nenv_key = \"" CCM_CODEX_ENV_NAME L"\"\r\n");
    jout_text(o, L"wire_api = \"responses\"\r\n");
}

/* Codex config.toml transform. model may be empty (keep/omit). set_provider
   writes the managed provider; clear_provider removes Manager-owned traces.
   Returns FALSE on any parse problem (invalid TOML, duplicate managed key,
   duplicate managed section) — callers must not write the file in that case. */
static BOOL codex_toml_transform(LPCWSTR input, BOOL exists, BOOL set_provider, BOOL clear_provider, LPCWSTR model, LPCWSTR effort, LPCWSTR base_url, LPWSTR output, unsigned int cap) {
    CCM_JSON_OUT out;
    unsigned int i = 0, n;
    int model_seen = 0, provider_seen = 0, effort_seen = 0, managed_emitted = 0;
    BOOL seen_header = FALSE, skip_section = FALSE;
    WCHAR key[256], value[4096], header[256], parsed[4096];
    BOOL want_model = model && model[0];
    BOOL want_effort = effort && effort[0];
    if (!output || cap < 2) return FALSE;
    if (!exists || wblank(input)) {
        jout_init(&out, output, cap);
        if (want_model) { jout_text(&out, L"model = "); toml_emit_string(&out, model); jout_text(&out, L"\r\n"); }
        if (want_effort) { jout_text(&out, L"model_reasoning_effort = "); toml_emit_string(&out, effort); jout_text(&out, L"\r\n"); }
        if (set_provider) {
            if (want_model || want_effort) jout_text(&out, L"\r\n");
            jout_text(&out, L"model_provider = \"" CCM_CODEX_PROVIDER_ID L"\"\r\n\r\n");
            toml_emit_managed_section(&out, base_url);
        }
        return out.ok;
    }
    if (!toml_validate(input)) return FALSE;
    jout_init(&out, output, cap);
    n = wlen(input);
    while (i < n) {
        unsigned int ls = i, le = i, k;
        BOOL emit_raw = TRUE;
        while (le < n && input[le] != L'\n') le++;
        if (toml_header_name(input, ls, le, header, 256)) {
            BOOL is_managed_hdr = weq(header, L"model_providers." CCM_CODEX_PROVIDER_ID);
            if (!seen_header) {
                /* First header: this is the last safe spot for pending
                   top-level insertions. When the first header IS the managed
                   section, only the pending keys flush here — the section is
                   regenerated by the branch below, never twice. */
                if (want_model && !model_seen) { jout_text(&out, L"model = "); toml_emit_string(&out, model); jout_text(&out, L"\r\n"); }
                if (want_effort && !effort_seen) { jout_text(&out, L"model_reasoning_effort = "); toml_emit_string(&out, effort); jout_text(&out, L"\r\n"); }
                if (set_provider && !provider_seen) jout_text(&out, L"model_provider = \"" CCM_CODEX_PROVIDER_ID L"\"\r\n");
                if (set_provider && !managed_emitted && !is_managed_hdr) { toml_emit_managed_section(&out, base_url); managed_emitted = 1; }
            }
            seen_header = TRUE;
            if (is_managed_hdr) {
                if (managed_emitted) return FALSE; /* duplicated managed section */
                skip_section = TRUE;
                if (set_provider) { toml_emit_managed_section(&out, base_url); managed_emitted = 1; }
                /* clear_provider: the whole section body is dropped. */
                emit_raw = FALSE;
            } else {
                skip_section = FALSE;
            }
        } else if (skip_section) {
            emit_raw = FALSE; /* inside the old managed section body */
        } else if (!seen_header && toml_line_key_value(input, ls, le, key, 256, value, 4096)) {
            if (weq(key, L"model")) {
                model_seen++;
                if (model_seen > 1) return FALSE;
                if (want_model) { jout_text(&out, L"model = "); toml_emit_string(&out, model); jout_text(&out, L"\r\n"); emit_raw = FALSE; }
            } else if (weq(key, L"model_reasoning_effort")) {
                effort_seen++;
                if (effort_seen > 1) return FALSE;
                if (want_effort) { jout_text(&out, L"model_reasoning_effort = "); toml_emit_string(&out, effort); jout_text(&out, L"\r\n"); emit_raw = FALSE; }
            } else if (weq(key, L"model_provider")) {
                provider_seen++;
                if (provider_seen > 1) return FALSE;
                if (set_provider) { jout_text(&out, L"model_provider = \"" CCM_CODEX_PROVIDER_ID L"\"\r\n"); emit_raw = FALSE; }
                else if (clear_provider && toml_parse_string_value(value, parsed, 4096) && weq(parsed, CCM_CODEX_PROVIDER_ID)) {
                    emit_raw = FALSE; /* drop only the managed assignment */
                }
            }
        }
        if (emit_raw) {
            for (k = ls; k < le; k++) jout_char(&out, input[k]);
            /* Replacement branches already append their own CRLF; skipped
               lines (old managed body) must not leave blank lines behind. */
            if (le < n) jout_char(&out, L'\n');
        }
        if (le < n) i = le + 1;
        else break;
    }
    if (!seen_header) {
        if (want_model && !model_seen) { jout_text(&out, L"model = "); toml_emit_string(&out, model); jout_text(&out, L"\r\n"); }
        if (want_effort && !effort_seen) { jout_text(&out, L"model_reasoning_effort = "); toml_emit_string(&out, effort); jout_text(&out, L"\r\n"); }
        if (set_provider && !provider_seen) { if (wlen(output)) jout_text(&out, L"\r\n"); jout_text(&out, L"model_provider = \"" CCM_CODEX_PROVIDER_ID L"\"\r\n"); }
        if (set_provider && !managed_emitted) { jout_char(&out, L'\n'); toml_emit_managed_section(&out, base_url); jout_text(&out, L"\r\n"); managed_emitted = 1; }
    }
    return out.ok;
}
...
static void manage_global_agent_teams(void) {
    ...
    }else if(pMessageBoxW(g_main,L"将移除 AgentDock 托管的多智能体规则，并恢复启用前的环境变量值。\r\n\r\n是否继续？",L"停用多智能体",MB_YESNO|MB_ICONQUESTION)!=IDYES)return;
...
static DWORD __stdcall import_thread(PVOID unused) {
    ...
    append_log(L"[OK] 配置导入完成；敏感字段未写入 settings.json。");append_log_raw(L"[OK] 目标文件：");append_log(g_import_target);
    append_log(L"[INFO] Claude Code 支持配置热重载：新配置对新建会话即时生效。");
    set_status(L"settings.json 导入完成");
done:
...
static void start_import_settings(void){
    ...
    if(th)CloseHandle(th);else{g_import_busy=0;set_busy(FALSE);secure_zero_w(g_import_source,4096);pMessageBoxW(g_main,L"无法启动配置导入线程。",CCM_APP_TITLE,MB_ICONERROR);}
}


static BOOL codex_config_path(LPWSTR out, unsigned int cap) {
    return agent_config_path(1, out, cap);
}

/* Config file path for any agent in the descriptor table. */
static BOOL agent_config_path(int app, LPWSTR out, unsigned int cap) {
    WCHAR profile[4096]; profile[0]=0;
    if(app<0||app>=CCM_AGENT_COUNT)return FALSE;
    if(!GetEnvironmentVariableW(L"USERPROFILE",profile,4096)||!profile[0])return FALSE;
    wcopy(out,cap,profile);
    wcat(out,cap,L"\\");wcat(out,cap,CCM_AGENTS[app].config_dir);
    wcat(out,cap,L"\\");wcat(out,cap,CCM_AGENTS[app].config_file);
    return TRUE;
}

/* Codex config.toml save. Same safety contract as the JSON import path:
   strict text read, fail-closed transform, timestamped raw backup, unique
   temp file, atomic MOVEFILE_WRITE_THROUGH replace. The API key is stored in
   Windows Credential Manager and referenced from config.toml via env_key, so
   no secret ever lands on disk and auth.json (which may hold ChatGPT OAuth
   tokens) is never touched. */
static DWORD __stdcall codex_save_thread(PVOID unused) {
    WCHAR target[4096],backup[4096],temp[4096],authp[4096],profile[4096],tick[32],num[32];
    BOOL existed; DWORD code=0;(void)unused;
    set_busy(TRUE);set_status(L"正在保存 Codex 配置...");
    append_log(L"");append_log(L"========================================");append_log(L"Codex config.toml 安全写入");append_log(L"========================================");
    if(!codex_config_path(target,4096)){append_log(L"[ERROR] 无法确定当前用户目录。");code=10;goto done;}
    existed=GetFileAttributesW(target)!=INVALID_FILE_ATTRIBUTES;
    if(existed){
        if(!read_text_file_w_strict(target,g_json_source_text,sizeof(g_json_source_text)/2)){append_log(L"[ERROR] config.toml 不是受支持的 UTF-8/UTF-16LE 文本或文件过大，未修改。");code=11;goto done;}
    } else g_json_source_text[0]=0;
    if(!codex_toml_transform(g_json_source_text,existed,g_codex_set_provider,TRUE,g_codex_model,g_codex_effort,g_codex_base,g_json_output_text,sizeof(g_json_output_text)/2)){
        append_log(L"[ERROR] config.toml 解析失败或存在重复的托管键，未修改任何文件。");code=12;goto done;
    }
    if(!ensure_parent_directory_native(target)){append_log(L"[ERROR] 无法创建 .codex 文件夹。");code=13;goto done;}
    if(existed){
        wcopy(backup,4096,target);wcat(backup,4096,L".backup-");uint_to_wstr((unsigned int)GetTickCount64(),tick,32);wcat(backup,4096,tick);
        if(!CopyFileW(target,backup,TRUE)){append_log(L"[ERROR] 无法创建写入前备份。");code=14;goto done;}
        append_log_raw(L"[INFO] 已创建备份：");append_log(backup);
    }
    wcopy(temp,4096,target);wcat(temp,4096,L".ccm.tmp");
    if(!write_utf8_text_file(temp,g_json_output_text)){append_log(L"[ERROR] 无法写入临时配置文件。");code=15;goto done;}
    if(!MoveFileExW(temp,target,MOVEFILE_REPLACE_EXISTING|MOVEFILE_WRITE_THROUGH)){DeleteFileW(temp);append_log(L"[ERROR] 无法原子替换 config.toml。");code=16;goto done;}
    if(g_codex_set_provider&&g_codex_secret[0]){
        if(!save_credential_native(CCM_CODEX_ENV_NAME,g_codex_secret)){append_log(L"[ERROR] 无法保存网关密钥到凭据管理器。");code=17;goto done;}
        if(!set_active_credential_names_native(CCM_CODEX_ENV_NAME))append_log(L"[WARN] 配置已保存，但活动凭据范围未能更新。");
        append_log_raw(L"[SECURE] 网关密钥已保存到 Windows 凭据管理器（");append_log_raw(CCM_CODEX_ENV_NAME);append_log(L"），未写入任何文件。");
        append_log(L"[INFO] 通过“启动 Codex”按钮启动时会自动注入密钥；在其它终端手动运行 codex 前请自行设置该环境变量。");
    }
    profile[0]=0;GetEnvironmentVariableW(L"USERPROFILE",profile,4096);
    if(profile[0]){
        wcopy(authp,4096,profile);wcat(authp,4096,L"\\.codex\\auth.json");
        if(file_exists_regular(authp)&&read_text_file_w(authp,g_codex_model,1024)){
            if(wfind_exact_from(g_codex_model,L"\"tokens\"",0)>=0)append_log(L"[INFO] 检测到 auth.json 中的 ChatGPT 登录态；本工具未做任何修改。");
            secure_zero_w(g_codex_model,1024);
        }
    }
    append_log_raw(L"[OK] Codex 配置已写入：");append_log(target);
    append_log(L"[INFO] Codex 在启动时读取配置：请重启 Codex 终端使本次切换生效。");
    set_status(L"Codex 配置已保存");
done:
    if(code){uint_to_wstr(code,num,32);append_log_raw(L"[ERROR] Codex 配置保存失败，错误代码：");append_log(num);set_status(L"Codex 配置保存失败");}
    secure_zero_w(g_codex_secret,4096);secure_zero_w(g_codex_base,2048);secure_zero_w(g_codex_model,1024);secure_zero_w(g_codex_effort,64);
    secure_zero_w(g_json_source_text,sizeof(g_json_source_text)/2);
    g_import_busy=0;set_busy(FALSE);return code;
}

/* ------------------------------------------------------------------------ */
/* Provider profiles (P2). Registry-backed, per target app.                   */
/* ------------------------------------------------------------------------ */

static const WCHAR PROFILE_LIST_PREFIX[] = L"Profiles";
#define CCM_PROFILE_SEP   0x1F   /* entry separator inside one REG_SZ */
#define CCM_PROFILE_FIELD L'\n'  /* field separator */

/* Registry value name per agent: "Profiles" + table short_name, i.e.
   ProfilesClaude / ProfilesCodex — stable across releases. */
static void profile_list_value(int app, LPWSTR out, unsigned int cap) {
    out[0]=0;wcat(out,cap,PROFILE_LIST_PREFIX);
    if(app>=0&&app<CCM_AGENT_COUNT)wcat(out,cap,CCM_AGENTS[app].short_name);
}

static void profile_slot_name(DWORD id, LPWSTR out, unsigned int cap) {
    WCHAR num[32];
    out[0]=0;wcat(out,cap,L"P");uint_to_wstr(id,num,32);wcat(out,cap,num);
}

static BOOL read_credential_value(LPCWSTR name, LPWSTR out, unsigned int cap) {
    WCHAR target[512]; CREDENTIALW_CCM *cred=0; unsigned int chars; BOOL ok=FALSE;
    out[0]=0;target[0]=0;wcat(target,512,L"AgentDock/");wcat(target,512,name);
    if(!(CredReadW(target,CRED_TYPE_GENERIC,0,&cred)&&cred&&cred->CredentialBlob&&cred->CredentialBlobSize>=2)){
        /* Credentials saved before the AgentDock rename live under the
           legacy "ClaudeCodeManager/" target prefix; fall back so users do
           not have to re-enter keys. */
        target[0]=0;wcat(target,512,L"ClaudeCodeManager/");wcat(target,512,name);
        if(cred){CredFree(cred);cred=0;}
        CredReadW(target,CRED_TYPE_GENERIC,0,&cred);
    }
    if(cred&&cred->CredentialBlob&&cred->CredentialBlobSize>=2){
        chars=cred->CredentialBlobSize/2;if(chars>=cap)chars=cap-1;
        memcpy(out,cred->CredentialBlob,chars*2);out[chars]=0;ok=out[0]!=0;
    }
    if(cred)CredFree(cred);
    return ok;
}

static BOOL profile_read_secret(DWORD id, LPWSTR out, unsigned int cap) {
    WCHAR slot[64];profile_slot_name(id,slot,64);return read_credential_value(slot,out,cap);
}

static BOOL profile_write_secret(DWORD id, LPCWSTR value) {
    WCHAR slot[64];profile_slot_name(id,slot,64);return save_credential_native(slot,value);
}

static void profile_delete_secret(DWORD id) {
    /* Overwrite the slot with an empty credential via CredWrite is not
       possible for deletion; CredDelete is not imported, so store a single
       space placeholder — profile_read_secret rejects blank values, which
       keeps the slot inert. */
    WCHAR slot[64];profile_slot_name(id,slot,64);save_credential_native(slot,L" ");
}

static void profiles_pack(void) {
    int i;
    WCHAR num[32];
    g_profile_pack[0]=0;
    for(i=0;i<g_profile_count;i++){
        CCM_PROFILE *p=&g_profiles[i];
        if(i)wcat(g_profile_pack,262144,L"\x1F");
        uint_to_wstr(p->id,num,32);
        wcat(g_profile_pack,262144,num);wcat(g_profile_pack,262144,L"\n");
        wcat(g_profile_pack,262144,p->name);wcat(g_profile_pack,262144,L"\n");
        uint_to_wstr((unsigned int)p->kind,num,32);
        wcat(g_profile_pack,262144,num);wcat(g_profile_pack,262144,L"\n");
        wcat(g_profile_pack,262144,p->base);wcat(g_profile_pack,262144,L"\n");
        wcat(g_profile_pack,262144,p->defm);wcat(g_profile_pack,262144,L"\n");
        wcat(g_profile_pack,262144,p->opus);wcat(g_profile_pack,262144,L"\n");
        wcat(g_profile_pack,262144,p->sonnet);wcat(g_profile_pack,262144,L"\n");
        wcat(g_profile_pack,262144,p->haiku);wcat(g_profile_pack,262144,L"\n");
        wcat(g_profile_pack,262144,p->subagent);wcat(g_profile_pack,262144,L"\n");
        wcat(g_profile_pack,262144,p->effort);
    }
}

static BOOL profiles_unpack(LPCWSTR packed) {
    int i=0;
    g_profile_count=0;
    if(!packed)return TRUE;
    while(packed[i] && g_profile_count<CCM_PROFILE_MAX){
        CCM_PROFILE *p=&g_profiles[g_profile_count];
        WCHAR field[2200];
        int f=0;
        memset(p,0,sizeof(*p));
        while(f<CCM_PROFILE_FIELDS){
            unsigned int o=0;
            while(packed[i] && packed[i]!=L'\n' && packed[i]!=CCM_PROFILE_SEP && o+1<2200) field[o++]=packed[i++];
            field[o]=0;
            if(packed[i]==L'\n')i++;
            switch(f){
                case 0:{DWORD v=0;unsigned int k=0;while(field[k]>=L'0'&&field[k]<=L'9'){v=v*10+(field[k]-L'0');k++;}p->id=v;break;}
                case 1:wcopy(p->name,64,field);break;
                case 2:p->kind=(field[0]>=L'0'&&field[0]<=L'2')?(field[0]-L'0'):1;break;
                case 3:wcopy(p->base,2048,field);break;
                case 4:wcopy(p->defm,1024,field);break;
                case 5:wcopy(p->opus,1024,field);break;
                case 6:wcopy(p->sonnet,1024,field);break;
                case 7:wcopy(p->haiku,1024,field);break;
                case 8:wcopy(p->subagent,1024,field);break;
                case 9:wcopy(p->effort,16,field);break;
            }
            f++;
        }
        /* Skip to the next entry separator (tolerates truncated tails). */
        while(packed[i] && packed[i]!=CCM_PROFILE_SEP)i++;
        if(packed[i]==CCM_PROFILE_SEP)i++;
        if(p->name[0] || p->defm[0] || p->base[0])g_profile_count++;
    }
    return TRUE;
}

static void profiles_load(int app) {
    HKEY key=0;WCHAR valuename[64];
    g_profile_count=0;g_profile_app=app;g_profile_pack[0]=0;
    profile_list_value(app,valuename,64);
    if(pRegOpenKeyExW(HKEY_CURRENT_USER,REG_KEY,0,KEY_READ,&key)==ERROR_SUCCESS){
        reg_read_string(key,valuename,g_profile_pack,131072);
        pRegCloseKey(key);
    }
    profiles_unpack(g_profile_pack);
}

static BOOL profiles_store(int app) {
    HKEY key=0;DWORD disp=0,bytes;BOOL ok;WCHAR valuename[64];
    profiles_pack();
    profile_list_value(app,valuename,64);
    if(pRegCreateKeyExW(HKEY_CURRENT_USER,REG_KEY,0,0,0,KEY_WRITE,0,&key,&disp)!=ERROR_SUCCESS)return FALSE;
    bytes=(wlen(g_profile_pack)+1)*2;
    ok=pRegSetValueExW(key,valuename,0,REG_SZ,(const BYTE*)g_profile_pack,bytes)==ERROR_SUCCESS;
    pRegCloseKey(key);
    return ok;
}

static DWORD profile_next_id(void) {
    DWORD maxid=0;int i;
    for(i=0;i<g_profile_count;i++)if(g_profiles[i].id>maxid)maxid=g_profiles[i].id;
    return maxid+1;
}

/* ---- Takeover: read the currently live config back into a profile ---- */

static BOOL wizard_read_live_claude(CCM_PROFILE *p, LPWSTR secret, unsigned int scap) {
    WCHAR settings[4096],names[512],name[256];
    HKEY rk=0;
    int root,env=-1,i,j;WCHAR key[256];
    secret[0]=0;memset(p,0,sizeof(*p));
    if(!agent_global_paths(settings,4096,settings,0))return FALSE;
    if(GetFileAttributesW(settings)==INVALID_FILE_ATTRIBUTES)return FALSE;
    if(!read_text_file_w_strict(settings,g_json_source_text,sizeof(g_json_source_text)/2))return FALSE;
    root=json_parse_document_native(g_json_source_text,g_json_source_tokens,&g_json_source_count);
    if(root<0||g_json_source_tokens[root].type!=CCM_JSON_OBJECT)return FALSE;
    j=root+1;
    for(i=0;i<g_json_source_tokens[root].size;i++){
        int k=j,v=k+1;key[0]=0;json_decode_string_token(g_json_source_text,&g_json_source_tokens[k],key,256);
        if(weq(key,L"env")&&g_json_source_tokens[v].type==CCM_JSON_OBJECT)env=v;
        else if(weq(key,L"model")&&p->defm[0]==0)token_value_to_w(g_json_source_text,&g_json_source_tokens[v],p->defm,1024);
        else if(weq(key,L"effortLevel"))token_value_to_w(g_json_source_text,&g_json_source_tokens[v],p->effort,16);
        j=json_token_after(g_json_source_tokens,v);
    }
    if(env>=0){
        j=env+1;
        for(i=0;i<g_json_source_tokens[env].size;i++){
            int k=j,v=k+1;key[0]=0;json_decode_string_token(g_json_source_text,&g_json_source_tokens[k],key,256);
            if(weq(key,L"ANTHROPIC_BASE_URL"))token_value_to_w(g_json_source_text,&g_json_source_tokens[v],p->base,2048);
            else if(weq(key,L"ANTHROPIC_MODEL")&&p->defm[0]==0)token_value_to_w(g_json_source_text,&g_json_source_tokens[v],p->defm,1024);
            else if(weq(key,L"ANTHROPIC_DEFAULT_OPUS_MODEL"))token_value_to_w(g_json_source_text,&g_json_source_tokens[v],p->opus,1024);
            else if(weq(key,L"ANTHROPIC_DEFAULT_SONNET_MODEL"))token_value_to_w(g_json_source_text,&g_json_source_tokens[v],p->sonnet,1024);
            else if(weq(key,L"ANTHROPIC_DEFAULT_HAIKU_MODEL"))token_value_to_w(g_json_source_text,&g_json_source_tokens[v],p->haiku,1024);
            else if(weq(key,L"CLAUDE_CODE_SUBAGENT_MODEL"))token_value_to_w(g_json_source_text,&g_json_source_tokens[v],p->subagent,1024);
            j=json_token_after(g_json_source_tokens,v);
        }
    }
    secure_zero_w(g_json_source_text,sizeof(g_json_source_text)/2);
    /* Prefill the active credential so the takeover can be saved as-is. */
    names[0]=0;
    if(pRegOpenKeyExW(HKEY_CURRENT_USER,REG_KEY,0,KEY_READ,&rk)==ERROR_SUCCESS){
        if(!reg_read_string(rk,L"ActiveCredentialNames",names,512))reg_read_string(rk,L"CredentialNames",names,512);
        pRegCloseKey(rk);
    }
    if(weq_ci(names,L"-"))names[0]=0;
    if(names[0]){
        unsigned int x=0;while(names[x]&&names[x]!=L';'&&x<256){name[x]=names[x];x++;}name[x]=0;
        if(name[0])read_credential_value(name,secret,scap);
    }
    wcopy(p->name,64,p->base[0]?L"网关":L"官方");
    p->kind=p->base[0]?1:(p->defm[0]?2:0);
    return p->base[0]||p->defm[0]||secret[0];
}

static BOOL wizard_read_live_codex(CCM_PROFILE *p, LPWSTR secret, unsigned int scap) {
    WCHAR target[4096];
    unsigned int i=0,n,prov_start=0,prov_len=0;
    BOOL seen_header=FALSE;
    WCHAR key[256],value[4096],header[256],parsed[4096],provider[256];
    secret[0]=0;memset(p,0,sizeof(*p));provider[0]=0;
    if(!codex_config_path(target,4096))return FALSE;
    if(GetFileAttributesW(target)==INVALID_FILE_ATTRIBUTES)return FALSE;
    if(!read_text_file_w_strict(target,g_json_source_text,sizeof(g_json_source_text)/2))return FALSE;
    if(!toml_validate(g_json_source_text)){secure_zero_w(g_json_source_text,sizeof(g_json_source_text)/2);return FALSE;}
    n=wlen(g_json_source_text);
    while(i<n){
        unsigned int ls=i,le=i;
        while(le<n && g_json_source_text[le]!=L'\n')le++;
        if(toml_header_name(g_json_source_text,ls,le,header,256)){
            seen_header=TRUE;
            if(wfind_exact_from(header,L"model_providers.",0)==0 && provider[0] && weq(header+16,provider)){
                /* located the active provider's section header; body scanned next */
                prov_start=1;prov_len=0;
            } else prov_start=0;
        } else if(!seen_header && toml_line_key_value(g_json_source_text,ls,le,key,256,value,4096)){
            if(weq(key,L"model")&&p->defm[0]==0&&toml_parse_string_value(value,parsed,4096))wcopy(p->defm,1024,parsed);
            else if(weq(key,L"model_reasoning_effort")&&toml_parse_string_value(value,parsed,4096))wcopy(p->effort,16,parsed);
            else if(weq(key,L"model_provider")&&toml_parse_string_value(value,parsed,4096))wcopy(provider,256,parsed);
        } else if(seen_header && prov_start && toml_line_key_value(g_json_source_text,ls,le,key,256,value,4096)){
            if(weq(key,L"base_url")&&toml_parse_string_value(value,parsed,4096)){wcopy(p->base,2048,parsed);prov_start=2;}
        }
        i=le<n?le+1:n;
    }
    secure_zero_w(g_json_source_text,sizeof(g_json_source_text)/2);
    p->kind=(provider[0]&&p->base[0])?1:(p->defm[0]?2:0);
    if(p->kind==1)read_credential_value(CCM_CODEX_ENV_NAME,secret,scap);
    wcopy(p->name,64,p->kind==1?L"网关":(p->kind==2?L"模型":L"官方"));
    (void)prov_len;
    return p->base[0]||p->defm[0]||secret[0];
}
...
static void open_settings_file(void) {
    ...
    int r=pMessageBoxW(g_main,L"要打开哪个应用的配置文件？\r\n\r\n“是”＝Claude Code（settings.json）\r\n“否”＝Codex（config.toml）\r\n“取消”＝返回",L"打开配置",MB_YESNOCANCEL|MB_ICONQUESTION);
    if(r==IDCANCEL)return;
    if(r==IDNO) {
        if(!codex_config_path(target,4096)){pMessageBoxW(g_main,L"无法确定当前用户目录。",CCM_APP_TITLE,MB_ICONERROR);return;}
        if(!ensure_parent_directory_native(target)){pMessageBoxW(g_main,L"无法创建 .codex 文件夹。",CCM_APP_TITLE,MB_ICONERROR);return;}
    } else {
        if(!choose_settings_target(target,4096,FALSE)) return;
        if(!ensure_settings_file(target)) {
            append_log(L"[ERROR] 无法创建或准备配置文件。");
            pMessageBoxW(g_main,L"无法创建或准备配置文件。\r\n\r\n请检查当前账户是否有权写入 .claude 文件夹。",CCM_APP_TITLE,MB_ICONERROR); return;
        }
    }
    ...
    if(!CreateProcessW(notepad,params,0,0,FALSE,0,0,workdir,&si,&pi)) {
        LONG_PTR rr=(LONG_PTR)pShellExecuteW(g_main,L"open",target,0,workdir,SW_SHOWNORMAL);
        if(rr<=32){WCHAR err[32];uint_to_wstr((unsigned int)GetLastError(),err,32);append_log_raw(L"[ERROR] 无法打开配置文件，系统错误：");append_log(err);pMessageBoxW(g_main,L"配置文件已经创建，但无法启动记事本或默认编辑器。",CCM_APP_TITLE,MB_ICONERROR);return;}
...
/* OpenAI Responses 兼容性探活（Codex 目标）。wire_api 现仅支持 "responses"，
 * 所以第三方网关必须提供 /v1/responses；用与 Messages 探测相同的最小请求体
 * 原则：真实 model + 单字符 input + 最小 max_output_tokens，不触发实质推理。
 * 认证走标准 Authorization: Bearer（OpenAI 风格）。 */
static void build_codex_responses_url(LPCWSTR base,LPWSTR out,unsigned int cap) {
    wcopy(out,cap,base);while(wlen(out)>0&&out[wlen(out)-1]==L'/')out[wlen(out)-1]=0;
    if(wends_ci(out,L"/v1"))wcat(out,cap,L"/responses");else wcat(out,cap,L"/v1/responses");
}

static void build_codex_probe_body(LPCWSTR model_name) {
    int p=0;
    WCHAR safe[256];
    LPCWSTR m;
    ccm_probe_body[0]=0;
    if(!model_name || !model_name[0]) m=L"gpt-5.1"; else m=model_name;
    wcopy(safe,256,m);
    /* Body: {"model":"<x>","input":"ping","max_output_tokens":16} */
    const char prefix[] = "{\"model\":\"";
    const char mid[]    = "\",\"input\":\"ping\",\"max_output_tokens\":16}";
    int i=0;
    while(prefix[i] && p+1<(int)sizeof(ccm_probe_body)) ccm_probe_body[p++]=prefix[i++];
    json_append_utf8_escape(ccm_probe_body,&p,(int)sizeof(ccm_probe_body),safe);
    i=0;
...
static BOOL http_probe_codex_responses(LPCWSTR url,LPCWSTR token,LPCWSTR proxy,DWORD *status,DWORD *error) {
    WCHAR host[1024],path[4096],proxy_norm[1024],headers[12288];
    INTERNET_PORT port; BOOL secure;
    DWORD sz,idx;
    HINTERNET session=0,connect=0,request=0;
    ...
    *status=0;*error=0;
    ...
    WinHttpAddRequestHeaders(request,headers,(DWORD)-1,WINHTTP_ADDREQ_FLAG_ADD|WINHTTP_ADDREQ_FLAG_REPLACE);
    secure_zero_w(headers,12288);
    if(!WinHttpSendRequest(request,0,(DWORD)-1,(PVOID)ccm_probe_body,CCM_PROBE_BODY_LEN,CCM_PROBE_BODY_LEN,0)){*error=GetLastError();goto done;}
    if(!WinHttpReceiveResponse(request,0)){*error=GetLastError();goto done;}
    sz=sizeof(DWORD);idx=0;
    if(!WinHttpQueryHeaders(request,WINHTTP_QUERY_STATUS_CODE|WINHTTP_QUERY_FLAG_NUMBER,0,status,&sz,&idx))*status=0;
    ok=TRUE;
done:
...
static void start_network_test(void){
    ...
    if(th)CloseHandle(th);else{g_network_busy=0;if(g_test_network)pEnableWindow(g_test_network,TRUE);wcopy(g_network_status,512,L"无法启动网络检测");set_status(L"网络检测未启动");pMessageBoxW(g_main,L"无法启动网络检测线程。",CCM_APP_TITLE,MB_ICONERROR);}
}
...
static void start_install(void) {
    ...
    if(!thread) {
        set_busy(FALSE);
        pMessageBoxW(g_main,g_claude_installed?L"无法启动更新线程。":L"无法启动安装线程。",CCM_APP_TITLE,MB_ICONERROR);
        return;
    }
    CloseHandle(thread);
}
...
static DWORD __stdcall model_discovery_thread(PVOID unused) {
    ...
    int protocol=(g_model_fetch_app>=0&&g_model_fetch_app<CCM_AGENT_COUNT)?CCM_AGENTS[g_model_fetch_app].protocol:0;
    LPCWSTR proto=(g_model_fetch_app>=0&&g_model_fetch_app<CCM_AGENT_COUNT)?CCM_AGENTS[g_model_fetch_app].probe_label:L"";
    ...
    if(protocol==1)build_codex_responses_url(base,messages_url,4096);else build_anthropic_messages_url(base,messages_url,4096);
    ...
    if(probe_model[0]){
        BOOL probed;
        if(protocol==1)build_codex_probe_body(probe_model);else build_probe_body(probe_model);
        status=error=0;
        probed=protocol==1
            ? http_probe_codex_responses(messages_url,g_model_fetch_secret,proxy,&status,&error)
            : http_probe_anthropic_messages(messages_url,g_model_fetch_secret,proxy,&status,&error);
        if(!probed){
            uint_to_wstr(error,num,32);append_log_raw(L"[WARN] ");append_log_raw(proto);append_log_raw(L" 兼容性检测发生网络错误：");append_log(num);
        } else {
            reachable=TRUE;
            if(status==401||status==403){authfail=TRUE;append_log_raw(L"[WARN] ");append_log_raw(proto);append_log(L" 接口拒绝了访问密钥。");}
            else if(status==404||status==405){g_model_gateway_protocol_state=2;append_log_raw(L"[ERROR] 服务器未提供所需的 ");append_log_raw(proto);append_log_raw(L" 接口（");append_log_raw(messages_url);append_log(L"）。");}
            else if((status>=200&&status<300)||status==400||status==422){compatible=TRUE;authfail=FALSE;g_model_gateway_protocol_state=1;if(status==400||status==422){append_log_raw(L"[INFO] ");append_log_raw(proto);append_log(L" 接口返回 400/422，路由存在，视为兼容。");}}
            else{g_model_gateway_protocol_state=1;append_log_raw(L"[WARN] ");append_log_raw(proto);append_log_raw(L" 接口返回非预期状态码：");uint_to_wstr(status,num,32);append_log(num);}
        }
    }
    if(models_found&&compatible){uint_to_wstr((unsigned int)g_model_count,num,32);wcopy(g_model_status,512,proto);wcat(g_model_status,512,L" 兼容 · 已获取 ");wcat(g_model_status,512,num);wcat(g_model_status,512,L" 个模型。");append_log_raw(L"[OK] ");append_log_raw(proto);append_log_raw(L" 接口兼容，已获取 ");append_log_raw(num);append_log(L" 个模型。");}
    else if(models_found){uint_to_wstr((unsigned int)g_model_count,num,32);wcopy(g_model_status,512,L"已获取 ");wcat(g_model_status,512,num);wcat(g_model_status,512,L" 个模型，但 ");wcat(g_model_status,512,proto);wcat(g_model_status,512,L" 兼容性未确认；仍可手动保存。");append_log_raw(L"[WARN] 模型列表可用，但 ");append_log_raw(proto);append_log(L" 兼容性未确认。");}
    else if(compatible){wcopy(g_model_status,512,proto);wcat(g_model_status,512,L" 接口兼容，但没有返回模型列表；可手动填写模型 ID。");append_log_raw(L"[WARN] ");append_log_raw(proto);append_log(L" 接口兼容，但未获取到模型列表。");}
...
static void start_model_discovery(BOOL notify_missing) {
    ...
    }
    {
        ...
        g_model_gateway_protocol_state=0;g_model_fetch_base[0]=g_model_fetch_secret[0]=g_model_fetch_model[0]=0;g_model_fetch_app=g_wiz_app_index;pGetWindowTextW(g_wiz_base_url,g_model_fetch_base,2048);pGetWindowTextW(g_wiz_secret,g_model_fetch_secret,4096);pGetWindowTextW(g_wiz_default_model,g_model_fetch_model,1024);pGetWindowTextW(g_proxy,g_cfg_proxy,1024);
...
static void wizard_update_app(void) {
    int idx=(int)pSendMessageW(g_wiz_app,CB_GETCURSEL,0,0);
    int i;
    BOOL codex;
    if(idx<0)idx=g_wiz_app_index;if(idx>=CCM_AGENT_COUNT)idx=0;if(idx<0)idx=0;
    g_wiz_app_index=idx;g_cfg_wiz_app=(DWORD)idx;
    codex=(idx==1); /* per-app semantics live in the branches below; a new
                       agent needs its own provider items here */
    g_model_count=0;g_model_gateway_protocol_state=0;
    /* The provider list follows the target app: Codex gateways must speak
       the Responses API, so the wording and validation change with it. */
    pSendMessageW(g_wiz_provider,CB_RESETCONTENT,0,0);
    if(codex){
        pSendMessageW(g_wiz_provider,CB_ADDSTRING,0,(LPARAM)L"官方 ChatGPT 登录");
        pSendMessageW(g_wiz_provider,CB_ADDSTRING,0,(LPARAM)L"第三方网关（Responses API，推荐）");
        pSendMessageW(g_wiz_provider,CB_ADDSTRING,0,(LPARAM)L"只配置模型，不修改接口");
        if(g_wiz_scope)pEnableWindow(g_wiz_scope,FALSE); /* Codex ignores project-level provider overrides */
    }else{
        pSendMessageW(g_wiz_provider,CB_ADDSTRING,0,(LPARAM)L"官方 Claude（账号登录或官方 API）");
        pSendMessageW(g_wiz_provider,CB_ADDSTRING,0,(LPARAM)L"第三方 / 自建网关（推荐）");
        pSendMessageW(g_wiz_provider,CB_ADDSTRING,0,(LPARAM)L"只配置模型，不修改接口");
        if(g_wiz_scope)pEnableWindow(g_wiz_scope,TRUE);
    }
    pSendMessageW(g_wiz_provider,CB_SETCURSEL,1,0);
    /* Codex has a single model: hide the Claude role table and the context
       metadata columns, keep 主力模型 + 思考强度. */
    for(i=0;i<MODEL_ROLE_COUNT;i++){
        BOOL show_row=!codex||i==0;
        HWND model=model_combo_for_role(i);
        if(model)ShowWindow(model,show_row?SW_SHOW:SW_HIDE);
        if(g_wiz_context_capacity[i])ShowWindow(g_wiz_context_capacity[i],codex?SW_HIDE:SW_SHOW);
        if(g_wiz_context_budget[i])ShowWindow(g_wiz_context_budget[i],codex?SW_HIDE:SW_SHOW);
        if(g_wiz_context_warning[i])ShowWindow(g_wiz_context_warning[i],codex?SW_HIDE:SW_SHOW);
        if(g_wiz_context_reserve[i])ShowWindow(g_wiz_context_reserve[i],codex?SW_HIDE:SW_SHOW);
        if(g_wiz_context_effort[i])ShowWindow(g_wiz_context_effort[i],show_row?SW_SHOW:SW_HIDE);
    }
    if(g_wizard){RECT cr;GetClientRect(g_wizard,&cr);g_dpi=g_wizard_dpi;layout_wizard_controls(cr.right-cr.left,cr.bottom-cr.top);}
    wizard_update_provider();
    g_wiz_profile_sel=-1;
    profiles_load(idx);
    wizard_profile_combo_rebuild();
    wizard_load_live_if_empty();
...
static void wizard_update_provider(void) {
    ...
    if(g_wiz_app_index==1){
        if(provider==CCM_PROVIDER_GATEWAY){pSendMessageW(g_wiz_base_url,EM_SETCUEBANNER,0,(LPARAM)L"https://gateway.example.com/v1");pSendMessageW(g_wiz_secret,EM_SETCUEBANNER,0,(LPARAM)L"CODEX_API_KEY 访问密钥");wcopy(g_model_status,512,L"填写地址与密钥后，将自动测试 Responses 兼容性并获取模型。");}
        else if(provider==CCM_PROVIDER_OFFICIAL){pSendMessageW(g_wiz_base_url,EM_SETCUEBANNER,0,(LPARAM)L"使用 ChatGPT 账号登录，无需填写");pSendMessageW(g_wiz_secret,EM_SETCUEBANNER,0,(LPARAM)L"无需填写");wcopy(g_model_status,512,L"官方登录无需网关；如需固定默认模型可填写模型 ID。");}
        else{pSendMessageW(g_wiz_base_url,EM_SETCUEBANNER,0,(LPARAM)L"保持现有接口配置");pSendMessageW(g_wiz_secret,EM_SETCUEBANNER,0,(LPARAM)L"无需填写");wcopy(g_model_status,512,L"仅写入 model / model_reasoning_effort，不修改接口配置。");}
    }
    else if(provider==CCM_PROVIDER_GATEWAY){pSendMessageW(g_wiz_base_url,EM_SETCUEBANNER,0,(LPARAM)L"" CCM_PROXY_HINT_URL);pSendMessageW(g_wiz_secret,EM_SETCUEBANNER,0,(LPARAM)L"访问密钥");wcopy(g_model_status,512,L"填写服务器地址和访问密钥后，将自动测试并获取模型。");}
...
static void wizard_apply_recommended(void) {
    ...
    g_model_count=0;
    if(g_wiz_app_index==1){
        pSetWindowTextW(g_wiz_base_url,L""); pSetWindowTextW(g_wiz_secret,L""); pSetWindowTextW(g_wiz_default_model,L"");
        pSendMessageW(g_wiz_context_effort[0],CB_SETCURSEL,1,0);
        wizard_update_provider();
        return;
    }
...
/* ---- Minimal modal text input (profile naming) ---- */
static HWND g_input_edit;
static WCHAR g_input_text[256];
static BOOL g_input_ok;

static LRESULT __stdcall input_wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch(msg){
        case WM_CREATE: {
            g_input_edit=CreateWindowExW(WS_EX_CLIENTEDGE,L"EDIT",L"",WS_CHILD|WS_VISIBLE|WS_TABSTOP|ES_AUTOHSCROLL,16,44,388,24,hwnd,(HMENU)101,g_instance,0);
            if(g_input_edit){
                pSendMessageW(g_input_edit,WM_SETFONT,(WPARAM)g_font_body,TRUE);
                pSendMessageW(g_input_edit,EM_SETLIMITTEXT,63,0);
            }
            {HWND ok=CreateWindowExW(0,L"BUTTON",L"保存",WS_CHILD|WS_VISIBLE|WS_TABSTOP|BS_DEFPUSHBUTTON,214,88,88,30,hwnd,(HMENU)IDOK,g_instance,0);
             HWND cancel=CreateWindowExW(0,L"BUTTON",L"取消",WS_CHILD|WS_VISIBLE|WS_TABSTOP|BS_PUSHBUTTON,314,88,88,30,hwnd,(HMENU)IDCANCEL,g_instance,0);
             if(ok)pSendMessageW(ok,WM_SETFONT,(WPARAM)g_font_button,TRUE);
             if(cancel)pSendMessageW(cancel,WM_SETFONT,(WPARAM)g_font_button,TRUE);}
            if(g_input_edit)SetFocus(g_input_edit);
            return 0;
        }
        case WM_COMMAND: {
            int id=(int)(wp&0xFFFF);
            if(id==IDOK||id==IDCANCEL){
                if(id==IDOK&&g_input_edit)GetWindowTextW(g_input_edit,g_input_text,256);
                g_input_ok=(id==IDOK);
                DestroyWindow(hwnd);
                return 0;
            }
...
static BOOL prompt_text_input(HWND parent, LPCWSTR title, LPCWSTR initial, LPWSTR out, unsigned int cap) {
    WNDCLASSEXW wc; HWND dlg; MSG msg; RECT pr; int px,py;
    static BOOL registered=FALSE;
    static HBRUSH dlg_brush=0;
    if(!registered){
        memset(&wc,0,sizeof(wc));wc.cbSize=sizeof(wc);wc.lpfnWndProc=input_wndproc;wc.hInstance=g_instance;wc.hCursor=LoadCursorW(0,IDC_ARROW);
        dlg_brush=CreateSolidBrush(rgb(255,255,255));wc.hbrBackground=dlg_brush;
        wc.lpszClassName=L"CCMInputBox";
        if(!pRegisterClassExW(&wc))return FALSE;
        registered=TRUE;
    }
    g_input_ok=FALSE;g_input_text[0]=0;g_input_edit=0;
    GetWindowRect(parent,&pr);
    px=pr.left+((pr.right-pr.left)/2)-210;py=pr.top+((pr.bottom-pr.top)/2)-80;
    dlg=pCreateWindowExW(WS_EX_DLGMODALFRAME,L"CCMInputBox",title,WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU,px,py,420,160,parent,0,g_instance,0);
    if(!dlg)return FALSE;
    pEnableWindow(parent,FALSE);
    pShowWindow(dlg,SW_SHOW);pUpdateWindow(dlg);
    if(initial&&initial[0]&&g_input_edit)pSetWindowTextW(g_input_edit,initial);
    while(pGetMessageW(&msg,0,0,0)>0){
        if(IsDialogMessageW(dlg,&msg))continue;
        pTranslateMessage(&msg);pDispatchMessageW(&msg);
        if(!IsWindow(dlg))break;
    }
    pEnableWindow(parent,TRUE);
    if(g_input_ok&&g_input_text[0]){wcopy(out,cap,g_input_text);return TRUE;}
    return FALSE;
}

/* ---- Profile wizard interactions ---- */

static void wizard_profile_combo_rebuild(void) {
    int i;
    pSendMessageW(g_wiz_profile,CB_RESETCONTENT,0,0);
    if(g_profile_count==0){
        pSendMessageW(g_wiz_profile,CB_ADDSTRING,0,(LPARAM)L"暂无档案");
        pSendMessageW(g_wiz_profile,CB_SETCURSEL,0,0);
        pEnableWindow(g_wiz_profile,FALSE);
    }else{
        for(i=0;i<g_profile_count;i++)pSendMessageW(g_wiz_profile,CB_ADDSTRING,0,(LPARAM)g_profiles[i].name);
        pEnableWindow(g_wiz_profile,TRUE);
        pSendMessageW(g_wiz_profile,CB_SETCURSEL,(WPARAM)(g_wiz_profile_sel>=0?g_wiz_profile_sel:0),0);
    }
    pEnableWindow(g_wiz_profile_del,g_profile_count>0);
}

static void wizard_apply_profile_controls(const CCM_PROFILE *p) {
    int ei;
    pSendMessageW(g_wiz_provider,CB_SETCURSEL,(WPARAM)p->kind,0);
    pSetWindowTextW(g_wiz_base_url,p->base);
    pSetWindowTextW(g_wiz_default_model,p->defm);
    pSetWindowTextW(g_wiz_opus_model,p->opus);
    pSetWindowTextW(g_wiz_sonnet_model,p->sonnet);
    pSetWindowTextW(g_wiz_haiku_model,p->haiku);
    pSetWindowTextW(g_wiz_subagent_model,p->subagent);
    ei=effort_index_from_code(p->effort);
    pSendMessageW(g_wiz_context_effort[0],CB_SETCURSEL,(WPARAM)ei,0);
}

static void wizard_profile_select(void) {
    int idx=(int)pSendMessageW(g_wiz_profile,CB_GETCURSEL,0,0);
    CCM_PROFILE *p; WCHAR secret[4096];
    if(idx<0||idx>=g_profile_count)return;
    g_wiz_profile_sel=idx;p=&g_profiles[idx];
    wizard_apply_profile_controls(p);
    if(profile_read_secret(p->id,secret,4096)){pSetWindowTextW(g_wiz_secret,secret);secure_zero_w(secret,4096);}
    else pSetWindowTextW(g_wiz_secret,L"");
    g_model_count=0;g_model_gateway_protocol_state=0;
    wcopy(g_model_status,512,L"已载入档案：");wcat(g_model_status,512,p->name);wcat(g_model_status,512,L"。确认后点击“保存配置”即可切换。");
    wizard_update_provider();
    InvalidateRect(g_wizard,0,FALSE);
}

static void wizard_profile_collect(CCM_PROFILE *p) {
    int kind=(int)pSendMessageW(g_wiz_provider,CB_GETCURSEL,0,0);
    int eff=(int)pSendMessageW(g_wiz_effort,CB_GETCURSEL,0,0);
    memset(p,0,sizeof(*p));
    p->kind=(kind<0||kind>2)?1:kind;
    pGetWindowTextW(g_wiz_base_url,p->base,2048);
    pGetWindowTextW(g_wiz_default_model,p->defm,1024);
    pGetWindowTextW(g_wiz_opus_model,p->opus,1024);
    pGetWindowTextW(g_wiz_sonnet_model,p->sonnet,1024);
    pGetWindowTextW(g_wiz_haiku_model,p->haiku,1024);
    pGetWindowTextW(g_wiz_subagent_model,p->subagent,1024);
    wcopy(p->effort,16,effort_code_from_index(eff<0?2:eff));
}

static void wizard_profile_save_as(void) {
    CCM_PROFILE p; WCHAR secret[4096],msg[256];
    BOOL update=g_wiz_profile_sel>=0&&g_wiz_profile_sel<g_profile_count;
    wizard_profile_collect(&p);
    if(p.kind==CCM_PROVIDER_GATEWAY&&!p.base[0]){pMessageBoxW(g_wizard,L"网关档案需要先填写服务器地址。",L"存为档案",MB_ICONWARNING);return;}
    if(update){
        p.id=g_profiles[g_wiz_profile_sel].id;
        wcopy(p.name,64,g_profiles[g_wiz_profile_sel].name);
        msg[0]=0;wcat(msg,256,L"将覆盖档案“");wcat(msg,256,p.name);wcat(msg,256,L"”的全部字段与密钥。\r\n\r\n是否继续？");
        if(pMessageBoxW(g_wizard,msg,L"更新档案",MB_YESNO|MB_ICONQUESTION)!=IDYES)return;
    }else{
        WCHAR name[64];
        name[0]=0;
        if(!prompt_text_input(g_wizard,L"保存为配置档案",L"",name,64))return;
        {unsigned int k;for(k=0;name[k];k++)if(name[k]==L'\n'||name[k]==(WCHAR)CCM_PROFILE_SEP){pMessageBoxW(g_wizard,L"档案名称不能包含换行或控制字符。",L"存为档案",MB_ICONWARNING);return;}}
        p.id=profile_next_id();
        wcopy(p.name,64,name);
    }
    pGetWindowTextW(g_wiz_secret,secret,4096);
    if(secret[0]){
        if(!profile_write_secret(p.id,secret)){secure_zero_w(secret,4096);pMessageBoxW(g_wizard,L"无法保存档案密钥到凭据管理器。",L"存为档案",MB_ICONERROR);return;}
    }else profile_delete_secret(p.id);
    secure_zero_w(secret,4096);pSetWindowTextW(g_wiz_secret,L"");
    if(update)g_profiles[g_wiz_profile_sel]=p;
    else{g_profiles[g_profile_count++]=p;g_wiz_profile_sel=g_profile_count-1;}
    profiles_store(g_wiz_app_index);
    wizard_profile_combo_rebuild();
    wcopy(g_model_status,512,L"档案已保存：");wcat(g_model_status,512,p.name);wcat(g_model_status,512,L"。选中档案并“保存配置”即可一键切换。");
    append_log_raw(L"[INFO] 已保存配置档案（");append_log_raw(g_wiz_app_index==1?L"Codex":L"Claude Code");append_log_raw(L"）：");append_log(p.name);
    InvalidateRect(g_wizard,0,FALSE);
}

static void wizard_profile_delete(void) {
    int idx=g_wiz_profile_sel,i;
    CCM_PROFILE *p; WCHAR msg[256];
    if(idx<0||idx>=g_profile_count)return;
    p=&g_profiles[idx];
    msg[0]=0;wcat(msg,256,L"删除档案“");wcat(msg,256,p->name);wcat(msg,256,L"”？\r\n\r\n其密钥槽位将被同时清空；当前生效配置不受影响。");
    if(pMessageBoxW(g_wizard,msg,L"删除档案",MB_YESNO|MB_ICONWARNING)!=IDYES)return;
    profile_delete_secret(p->id);
    for(i=idx;i<g_profile_count-1;i++)g_profiles[i]=g_profiles[i+1];
    g_profile_count--;g_wiz_profile_sel=-1;
    profiles_store(g_wiz_app_index);
    wizard_profile_combo_rebuild();
    wcopy(g_model_status,512,L"档案已删除。");
    InvalidateRect(g_wizard,0,FALSE);
}

/* Takeover: with an empty profile list and an untouched form, read the live
   config (settings.json / config.toml + active credential) so the user can
   adopt the current setup as the first profile. */
static void wizard_load_live_if_empty(void) {
    CCM_PROFILE live; WCHAR secret[4096];
    WCHAR base[2048],defm[1024];
    BOOL ok;
    if(g_profile_count>0)return;
    base[0]=0;defm[0]=0;
    pGetWindowTextW(g_wiz_base_url,base,2048);pGetWindowTextW(g_wiz_default_model,defm,1024);
    if(base[0]||defm[0])return;
    memset(&live,0,sizeof(live));
    if(g_wiz_app_index==1)ok=wizard_read_live_codex(&live,secret,4096);
    else ok=wizard_read_live_claude(&live,secret,4096);
    if(!ok){secure_zero_w(secret,4096);return;}
    wizard_apply_profile_controls(&live);
    if(secret[0]){pSetWindowTextW(g_wiz_secret,secret);secure_zero_w(secret,4096);}
    g_model_count=0;g_model_gateway_protocol_state=0;
    wcopy(g_model_status,512,L"已读取当前生效配置；点击“存为档案”即可接管为第一个配置档案。");
    wizard_update_provider();
    InvalidateRect(g_wizard,0,FALSE);
}
...
static void wizard_save_codex(void) {
    WCHAR base[2048],secret[4096],defm[1024];
    WCHAR parsed_host[1024],parsed_path[4096];INTERNET_PORT parsed_port;BOOL parsed_secure;
    int provider,eff;HANDLE th;DWORD tid;
    provider=(int)pSendMessageW(g_wiz_provider,CB_GETCURSEL,0,0);eff=(int)pSendMessageW(g_wiz_effort,CB_GETCURSEL,0,0);
    if(provider<0)provider=CCM_PROVIDER_GATEWAY; if(eff<0)eff=2;
    base[0]=secret[0]=defm[0]=0;
    pGetWindowTextW(g_wiz_base_url,base,2048);pGetWindowTextW(g_wiz_secret,secret,4096);pGetWindowTextW(g_wiz_default_model,defm,1024);
    if(provider==CCM_PROVIDER_GATEWAY){
        int at,url_at;
        if(!base[0]){pMessageBoxW(g_wizard,L"请填写网关或第三方接口地址。\r\n\r\n示例：https://gateway.example.com/v1",L"还差一步",MB_ICONWARNING);return;}
        if(!url_parse_basic(base,parsed_host,1024,parsed_path,4096,&parsed_port,&parsed_secure)){pMessageBoxW(g_wizard,L"服务器地址无效。请填写以 http:// 或 https:// 开头的完整地址。",L"地址格式不正确",MB_ICONWARNING);return;}
        url_at=wfind_ci(base,L"://");at=wfind_ci(base,L"@");
        if(at>=0&&at>url_at){pMessageBoxW(g_wizard,L"服务器地址不应包含用户名或密码片段。\r\n\r\n请只填写 https://host[:port] 形式，访问密钥在下方单独填写。",L"地址格式不正确",MB_ICONWARNING);return;}
        if(g_model_gateway_protocol_state==2&&g_model_count==0){pMessageBoxW(g_wizard,L"该服务器可以列出模型，但缺少 Codex 必需的 Responses API（/v1/responses）。\r\n\r\nCodex 的 wire_api 仅支持 responses；请改用兼容的网关。",L"接口不兼容",MB_ICONERROR);return;}
        if(g_model_gateway_protocol_state==0&&g_model_count==0&&pMessageBoxW(g_wizard,L"尚未确认服务器是否兼容 OpenAI Responses API。\r\n\r\n建议先点击“测试并获取模型”。仍要保存吗？",L"兼容性尚未验证",MB_YESNO|MB_ICONWARNING)!=IDYES)return;
    }
    if(provider!=CCM_PROVIDER_OFFICIAL&&!defm[0]){pMessageBoxW(g_wizard,L"请填写模型 ID（如 gpt-5.1、gpt-5.1-codex）。",L"还差一步",MB_ICONWARNING);return;}
    if(provider==CCM_PROVIDER_GATEWAY&&!secret[0]){
        if(pMessageBoxW(g_wizard,L"尚未填写访问密钥。可以先保存配置，但通过“启动 Codex”运行前需要设置该密钥。\r\n\r\n仍要继续吗？",L"未填写访问密钥",MB_YESNO|MB_ICONWARNING)!=IDYES)return;
    }
    wcopy(g_codex_model,1024,defm);
    wcopy(g_codex_base,2048,provider==CCM_PROVIDER_GATEWAY?base:L"");
    wcopy(g_codex_secret,4096,provider==CCM_PROVIDER_GATEWAY?secret:L"");
    wcopy(g_codex_effort,64,effort_code_from_index(eff));
    g_codex_set_provider=(provider==CCM_PROVIDER_GATEWAY);
    append_log(L"");append_log(L"[INFO] Codex 配置正在保存；接口与模型写入 ~/.codex/config.toml，密钥仅存入凭据管理器。");
    secure_zero_w(secret,4096);pSetWindowTextW(g_wiz_secret,L"");wizard_hide();g_import_busy=1;set_busy(TRUE);
    th=CreateThread(0,0,codex_save_thread,0,0,&tid);
    if(th)CloseHandle(th);
    else{
        g_import_busy=0;set_busy(FALSE);
        secure_zero_w(g_codex_model,1024);secure_zero_w(g_codex_base,2048);secure_zero_w(g_codex_secret,4096);secure_zero_w(g_codex_effort,64);
        pMessageBoxW(g_main,L"无法启动 Codex 配置保存任务。",L"保存失败",MB_ICONERROR);
    }
}
...
static void wizard_save(void) {
    ...
    HANDLE th; DWORD tid;
    if(g_installing||g_import_busy) return;
    if(g_wiz_app_index==1){wizard_save_codex();return;}
...
static void layout_wizard_controls(int width, int height) {
    ...
    int i,y;
    BOOL codex=g_wiz_app_index==1;
    if(field_w<sc(320)) field_w=sc(320);
    pMoveWindow(g_wiz_app,inner_x,sc(93),inner_w,sc(140),TRUE);
    pMoveWindow(g_wiz_profile,inner_x,sc(135),inner_w-sc(176),sc(140),TRUE);
    pMoveWindow(g_wiz_profile_save,field_right-sc(168),sc(131),sc(96),sc(34),TRUE);
    pMoveWindow(g_wiz_profile_del,field_right-sc(64),sc(131),sc(64),sc(34),TRUE);
    pMoveWindow(g_wiz_scope,inner_x,sc(177),inner_w,sc(140),TRUE);
    pMoveWindow(g_wiz_provider,inner_x,sc(219),inner_w,sc(140),TRUE);
    pMoveWindow(g_wiz_base_url,field_x+sc(4),sc(260),field_w-sc(8),sc(24),TRUE);
    pMoveWindow(g_wiz_secret,secret_inner_x,sc(302),secret_inner_w,sc(24),TRUE);
    pMoveWindow(g_wiz_test_models,field_right-test_w,sc(296),test_w,row_h,TRUE);
    for(i=0;i<MODEL_ROLE_COUNT;i++){
        if(codex&&i>0)continue;
        y=sc(404+i*42);
        if(codex){
            /* Codex: one model row; the combo stretches to the effort column
               and the context metadata controls stay hidden. */
            pMoveWindow(g_wiz_default_model,model_x+sc(4),y+sc(7),effort_x-model_x-gap-sc(8),sc(160),TRUE);
            pMoveWindow(g_wiz_context_effort[0],effort_x+sc(4),y+sc(7),effort_w-sc(8),sc(120),TRUE);
        }else{
...
static void paint_wizard(HWND hwnd) {
    ...
    r=mkrect(sc(26),sc(46),c.right-sc(24),sc(70));
    text_out(dc,codex?L"配置 Codex CLI 的接口与模型；第三方网关需支持 Responses API。":L"设置原生 Claude Code 接口与模型；容量和预算仅用于本地规划。",r,g_wiz_font_subtitle,rgb(105,105,110),DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);

    r=mkrect(sc(24),sc(78),c.right-sc(24),sc(362)); fill_round(dc,r,sc(15),rgb(255,255,255),rgb(232,232,235));
    r=mkrect(sc(24),sc(372),c.right-sc(24),sc(638)); fill_round(dc,r,sc(15),rgb(255,255,255),rgb(232,232,235));
    ...
    r=mkrect(field_x,sc(212),field_right,sc(246)); fill_round(dc,r,sc(10),rgb(247,247,249),rgb(247,247,249));
    r=mkrect(field_x,sc(254),field_right,sc(288)); fill_round(dc,r,sc(10),rgb(247,247,249),rgb(247,247,249));
    r=mkrect(field_x,sc(296),field_right-sc(132),sc(330)); fill_round(dc,r,sc(10),rgb(247,247,249),rgb(247,247,249));

    r=mkrect(label_x,sc(86),label_right,sc(120)); text_out(dc,L"目标应用",r,g_wiz_font_button,rgb(45,45,48),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    r=mkrect(label_x,sc(128),label_right,sc(162)); text_out(dc,L"配置档案",r,g_wiz_font_button,rgb(45,45,48),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    r=mkrect(label_x,sc(170),label_right,sc(204)); text_out(dc,codex?L"保存范围（全局）":L"保存范围",r,g_wiz_font_button,rgb(45,45,48),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    r=mkrect(label_x,sc(212),label_right,sc(246)); text_out(dc,L"接口类型",r,g_wiz_font_button,rgb(45,45,48),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    r=mkrect(label_x,sc(254),label_right,sc(288)); text_out(dc,L"服务器地址",r,g_wiz_font_button,rgb(45,45,48),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    r=mkrect(label_x,sc(296),label_right,sc(330)); text_out(dc,L"访问密钥",r,g_wiz_font_button,rgb(45,45,48),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    r=mkrect(label_x,sc(332),c.right-sc(40),sc(360)); text_out(dc,g_model_status,r,g_wiz_font_subtitle,g_model_fetch_busy?rgb(216,119,86):rgb(105,105,110),DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);

    if(codex){
        r=mkrect(model_x,sc(376),effort_x-gap,sc(400));text_out(dc,L"模型 ID",r,g_wiz_font_subtitle,rgb(105,105,110),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
        r=mkrect(effort_x,sc(376),effort_x+effort_w,sc(400));text_out(dc,L"思考",r,g_wiz_font_subtitle,rgb(105,105,110),DT_CENTER|DT_VCENTER|DT_SINGLELINE);
        y=sc(404);
        r=mkrect(model_x,y,field_right,y+sc(34));fill_round(dc,r,sc(9),rgb(249,249,250),rgb(249,249,250));
        r=mkrect(table_left,y,model_x-sc(7),y+sc(34));text_out(dc,L"主力模型",r,g_wiz_font_button,rgb(45,45,48),DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
        r=mkrect(table_left,sc(614),c.right-sc(40),sc(636));text_out(dc,L"密钥经环境变量 CODEX_API_KEY 注入，不写入 config.toml；切换后需重启 Codex 终端生效。",r,g_wiz_font_subtitle,rgb(105,105,110),DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
    }else{
        r=mkrect(table_left,sc(376),model_x,sc(400));text_out(dc,L"角色",r,g_wiz_font_subtitle,rgb(105,105,110),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
        r=mkrect(model_x,sc(376),model_x+model_w,sc(400));text_out(dc,L"模型 ID",r,g_wiz_font_subtitle,rgb(105,105,110),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
        r=mkrect(cap_x,sc(376),cap_x+cap_w,sc(400));text_out(dc,L"容量",r,g_wiz_font_subtitle,rgb(105,105,110),DT_CENTER|DT_VCENTER|DT_SINGLELINE);
        r=mkrect(budget_x,sc(376),budget_x+budget_w,sc(400));text_out(dc,L"建议预算",r,g_wiz_font_subtitle,rgb(105,105,110),DT_CENTER|DT_VCENTER|DT_SINGLELINE);
        r=mkrect(warning_x,sc(376),warning_x+warning_w,sc(400));text_out(dc,L"预警 %",r,g_wiz_font_subtitle,rgb(105,105,110),DT_CENTER|DT_VCENTER|DT_SINGLELINE);
        r=mkrect(reserve_x,sc(376),reserve_x+reserve_w,sc(400));text_out(dc,L"输出预留",r,g_wiz_font_subtitle,rgb(105,105,110),DT_CENTER|DT_VCENTER|DT_SINGLELINE);
        r=mkrect(effort_x,sc(376),effort_x+effort_w,sc(400));text_out(dc,L"思考",r,g_wiz_font_subtitle,rgb(105,105,110),DT_CENTER|DT_VCENTER|DT_SINGLELINE);
        for(i=0;i<MODEL_ROLE_COUNT;i++){
            y=sc(404+i*42);r=mkrect(model_x,y,field_right,y+sc(34));fill_round(dc,r,sc(9),rgb(249,249,250),rgb(249,249,250));
        ...
        r=mkrect(table_left,sc(614),c.right-sc(40),sc(636));text_out(dc,L"容量可自动识别或手动标注；本地预算不改变 Claude Code 或网关限制。",r,g_wiz_font_subtitle,rgb(105,105,110),DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
    }
    EndPaint(hwnd,&ps);
}
...
static LRESULT __stdcall wizard_wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch(msg) {
        case WM_CREATE:
            ...
            g_wiz_app=create_child_control(hwnd,0,L"COMBOBOX",L"",WS_TABSTOP|WS_VSCROLL|CBS_DROPDOWNLIST,0,0,0,0,IDC_WIZ_APP);
            {int ai;for(ai=0;ai<CCM_AGENT_COUNT;ai++)pSendMessageW(g_wiz_app,CB_ADDSTRING,0,(LPARAM)CCM_AGENTS[ai].display);}
            g_wiz_app_index=(g_cfg_wiz_app==1)?1:0;
            pSendMessageW(g_wiz_app,CB_SETCURSEL,(WPARAM)g_wiz_app_index,0);
            g_wiz_profile=create_child_control(hwnd,0,L"COMBOBOX",L"",WS_TABSTOP|WS_VSCROLL|CBS_DROPDOWNLIST,0,0,0,0,IDC_WIZ_PROFILE);
            g_wiz_profile_save=create_child_control(hwnd,0,L"BUTTON",L"存为档案",WS_TABSTOP|BS_OWNERDRAW,0,0,0,0,IDC_WIZ_PROFILE_SAVE);
            g_wiz_profile_del=create_child_control(hwnd,0,L"BUTTON",L"删除",WS_TABSTOP|BS_OWNERDRAW,0,0,0,0,IDC_WIZ_PROFILE_DEL);
            g_wiz_profile_sel=-1;
            ...
            apply_wizard_control_fonts();apply_wizard_edit_margins();load_context_metadata_controls();{RECT cr;GetClientRect(hwnd,&cr);g_dpi=g_wizard_dpi;layout_wizard_controls(cr.right-cr.left,cr.bottom-cr.top);} wizard_update_app(); return 0;
        ...
        case WM_GETMINMAXINFO: {MINMAXINFO *mmi=(MINMAXINFO*)lp;g_dpi=g_wizard_dpi;if(mmi){mmi->ptMinTrackSize_x=sc(880);mmi->ptMinTrackSize_y=sc(748);return 0;}break;}
        ...
        case WM_COMMAND: {
            ...
            if(id==IDC_WIZ_APP&&code==CBN_SELCHANGE){wizard_update_app();return 0;}
            if(id==IDC_WIZ_PROFILE&&code==CBN_SELCHANGE){wizard_profile_select();return 0;}
            if(id==IDC_WIZ_PROFILE_SAVE&&code==BN_CLICKED){wizard_profile_save_as();return 0;}
            if(id==IDC_WIZ_PROFILE_DEL&&code==BN_CLICKED){wizard_profile_delete();return 0;}
...
static void show_model_wizard(void) {
    ...
    if(!g_wizard) {
        g_wizard=pCreateWindowExW(WS_EX_CONTROLPARENT,WIZARD_CLASS_NAME,L"模型配置",WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MAXIMIZEBOX|WS_THICKFRAME|WS_CLIPCHILDREN|WS_CLIPSIBLINGS,
            CW_USEDEFAULT,CW_USEDEFAULT,sc(1000),sc(768),g_main,0,g_instance,0);
        if(!g_wizard){pMessageBoxW(g_main,L"无法打开模型配置向导。",CCM_APP_TITLE,MB_ICONERROR);return;}
...
static void layout_controls(int width, int height) {
    ...
    /* One row: primary Claude launch beside Codex launch and folder open. */
    {int third_w=(left_btn_area-btn_gap*2)/3;
     int row_y=top+sc(compact_left?296:336),row_h=compact_left?sc(36):sc(40);
     pMoveWindow(g_launch,lx+inner,row_y,third_w,row_h,TRUE);
     pMoveWindow(g_launch_codex,lx+inner+third_w+btn_gap,row_y,third_w,row_h,TRUE);
     pMoveWindow(g_folder,lx+inner+(third_w+btn_gap)*2,row_y,left_btn_area-(third_w+btn_gap)*2,row_h,TRUE);}

    /* Right card */
    pMoveWindow(g_network_mode,rx+inner,top+sc(120),network_w,sc(120),TRUE);
    {RECT combo_rect;int combo_h=sc(24);if(GetWindowRect(g_network_mode,&combo_rect)){int actual=(int)(combo_rect.bottom-combo_rect.top);if(actual>=sc(20)&&actual<=sc(32))combo_h=actual;}pMoveWindow(g_test_network,rx+inner+network_w+network_gap,top+sc(120),test_w,combo_h,TRUE);}
    pMoveWindow(g_proxy,rx+sc(32),top+sc(183),rightw-sc(64),sc(20),TRUE);
    pMoveWindow(g_url,rx+sc(32),top+sc(235),rightw-sc(64),sc(20),TRUE);
    pMoveWindow(g_install,rx+inner,top+sc(268),pair_w,sc(36),TRUE);
    pMoveWindow(g_shortcut,rx+inner+pair_w+sc(10),top+sc(268),right_inner_w-pair_w-sc(10),sc(36),TRUE);
    pMoveWindow(g_import_settings,rx+inner,top+sc(310),pair_w,sc(32),TRUE);
    pMoveWindow(g_open_settings,rx+inner+pair_w+sc(10),top+sc(310),right_inner_w-pair_w-sc(10),sc(32),TRUE);
    pMoveWindow(g_model_wizard,rx+inner,top+sc(346),pair_w,sc(32),TRUE);
    pMoveWindow(g_agent_teams,rx+inner+pair_w+sc(10),top+sc(346),right_inner_w-pair_w-sc(10),sc(32),TRUE);
...
static void paint_main(HWND hwnd) {
    ...
    r=mkrect(rx+sc(20),top+sc(96),rx+rightw-sc(20),top+sc(114));text_out(dc,L"网络环境",r,g_font_subtitle,rgb(100,100,105),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    r=mkrect(rx+sc(20),top+sc(144),rx+rightw-sc(20),top+sc(160));text_out(dc,g_network_status,r,g_font_subtitle,g_network_busy?rgb(216,119,86):rgb(125,125,130),DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
    r=mkrect(rx+sc(20),top+sc(160),rx+rightw-sc(20),top+sc(178));text_out(dc,L"自定义代理地址（仅自定义模式可编辑）",r,g_font_subtitle,rgb(120,120,125),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    r=mkrect(rx+sc(20),top+sc(178),rx+rightw-sc(20),top+sc(208));fill_round(dc,r,sc(9),rgb(247,247,249),rgb(232,232,235));
    r=mkrect(rx+sc(20),top+sc(212),rx+rightw-sc(20),top+sc(230));text_out(dc,L"安装来源（通常无需修改）",r,g_font_subtitle,rgb(120,120,125),DT_LEFT|DT_VCENTER|DT_SINGLELINE);
    r=mkrect(rx+sc(20),top+sc(230),rx+rightw-sc(20),top+sc(260));fill_round(dc,r,sc(9),rgb(247,247,249),rgb(232,232,235));
...
static LRESULT __stdcall wndproc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    switch (msg) {
        case WM_CREATE: {
            ...
            g_launch_codex=create_control(0,L"BUTTON",L"启动 Codex",WS_TABSTOP|BS_OWNERDRAW,0,0,0,0,IDC_LAUNCH_CODEX);
            ...
            append_log(L"双应用支持：向导顶部可切换 Claude Code / Codex；Codex 密钥经 CODEX_API_KEY 注入。");
        ...
        case WM_COMMAND: {
            ...
            else if(id==IDC_LAUNCH && code==BN_CLICKED){save_config();launch_in_console(CCM_LAUNCH_CLAUDE);}
            else if(id==IDC_LAUNCH_CODEX && code==BN_CLICKED){save_config();launch_in_console(CCM_LAUNCH_CODEX);}
            ...
            else if(id==IDC_PROJECTS && code==LBN_DBLCLK){save_config();launch_in_console(CCM_LAUNCH_CLAUDE);}
            return 0;
        }
        case WM_CLOSE:
            if(g_installing){pMessageBoxW(hwnd,L"任务仍在进行，请任务结束后再关闭。",CCM_APP_TITLE,0x30);return 0;}
            save_config(); break;
        case WM_DESTROY:
...
void mainCRTStartup(void) {
    ...
    if(extract_launcher_argument(L"--claude-launcher",launcher_project,4096)) ExitProcess(run_unified_console_launcher(CCM_LAUNCH_CLAUDE,launcher_project));
    if(extract_launcher_argument(L"--codex-launcher",launcher_project,4096)) ExitProcess(run_unified_console_launcher(CCM_LAUNCH_CODEX,launcher_project));
    if(extract_launcher_argument(L"--terminal-launcher",launcher_project,4096)) ExitProcess(run_unified_console_launcher(CCM_LAUNCH_TERMINAL,launcher_project));
    startup_log_open();
    startup_log_write("mainCRTStartup entered\r\n");
    if(!load_apis()) { startup_log_write("load_apis failed\r\n"); MessageBoxW(0,L"Failed to initialize Windows APIs.",CCM_APP_TITLE,0x10); startup_log_close(); ExitProcess(2); }
    ...
    if(!hwnd) { startup_log_write("CreateWindowExW failed\r\n"); MessageBoxW(0,L"Unable to create the application window. See %TEMP%\\AgentDock-startup.log.",CCM_APP_TITLE,0x10); startup_log_close(); ExitProcess(4); }
